...
# plus the runtime backstop cover that); sibylfs/generated and
# lean_frontend/generated carry no stamp (different failure surfaces,
# own gates).
#
# Usage: tools/check_lem_sync.sh [--root DIR] --record|--check
...
[[ -n "$MODE" ]] || { echo "usage: check_lem_sync.sh [--root DIR] --record|--check" >&2; exit 2; }
