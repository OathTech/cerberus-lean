...
cite-less register row), and on an obligation with a binder that is neither
reserved nor a wrapper argument (audit F-A4); `--selftest` plants five
doctored tables, three doctored registers and four compiled decoy obligations
(one under a contradictory hypothesis: MEASURED by shape, RED by the register;
one with an extra Prop binder: MALFORMED). The ambient
