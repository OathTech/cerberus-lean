...
- the digest boundary (`CerberusFresh.digest`/`forceIO`/`md5Hex`) —
  ...
  unfold them);
- (`CerbGlobal` config/switch refs — LEFT the boundary 2026-09-05: the
  refs were never written, so the eleven reads are now plain `def`s of
  the driver's default configuration, kernel-transparent, with `rfl`
  lemmas; `docs/2026-09-05_cerbglobal-defs-record.md`. Step 2 — the
  configuration as a reader-lifted parameter — is a separate slice;)
