...
| `check_pin_sites.sh` | the lem-lean pin is ONE value at every site that names it — the manifest `lem-pin`, the Lake `LemLib` rev, the three lake-manifests (`rev` + `inputRev`) and the README's newcomer `opam pin` command (fresh-clone finding 2026-09-25; 5 plants) |
