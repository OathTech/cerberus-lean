...
/-- Parse a cabs-json document: the translation unit AND its `digest` —
    the oracle's `Digest.file` of the C source, in `Digest.to_hex` form
    (backend/driver/main.ml --cabs-json path, which sets it with
    `Cerb_fresh.set_digest filename` exactly as the OCaml c_frontend does,
    pipeline.ml:181). Main.lean installs it as the TU's digest before the
    frontend stages, so every symbol the Lean pipeline mints for this TU
    carries the same digest as the oracle's (zero-discrepancy Z3, census
    row Z-28: symbol digests order the linked globals, hence the
    addresses). REQUIRED, fail-closed: a document without the field is a
    stale bridge artifact (generated by an older oracle), never a TU to run. -/
def parseJson (input : String) : Except String (String × translation_unit) := do
  let json ← Json.parse input |>.mapError toString
  let tunit ← jsonToTranslationUnit json
  let digest ← match getField json "digest" with
    | .ok (.str d) =>
      if d.length == 32 && d.all (fun c => c.isDigit || ('a' ≤ c && c ≤ 'f')) then .ok d
      else err "parseJson" s!"'digest' is not a 32-character lowercase-hex MD5: {d}"
    | .ok j => err "parseJson" s!"'digest' is not a string: {j}"
    | .error _ => err "parseJson" "missing 'digest' field — this cabs-json predates the zero-discrepancy Z3 bridge (regenerate it with the current oracle's --cabs-json; the Lean driver installs the source digest it carries as the TU digest, Cerb_fresh.set_digest mirror)"
  .ok (digest, tunit)
