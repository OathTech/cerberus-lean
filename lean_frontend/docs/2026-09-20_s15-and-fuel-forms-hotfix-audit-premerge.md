...
## 6. Delta re-read (2026-09-20, later the same day) — the two fix commits, the rebase, the final verdicts

**What changed since §0–§5.** (1) S1.5 cerberus: the orchestrator's `8c712657f` on `fix/s15-record-erratum` (from `52af8ccf1`) — ONE file, the S1.5 record, `+22` lines: a dated erratum section. The range to land is now `e283bed77..8c712657f` (4 commits). (2) The hotfix was REBASED onto `8c712657f` (`b89a010ab` charter, `5d462da86` hotfix) and ONE fix commit `e5065fc5a` was added (7 files). The range to land is `8c712657f..e5065fc5a` (3 commits). Nothing in lem-lean moved. All measurements below are mine unless marked read-only; the heavy ones ran in my hotfix audit worktree checked out DETACHED at `e5065fc5a` (returned to `audit/fuel-forms-carriers` before this commit).

### 6.1 Range 2 — `e283bed77..8c712657f`: M1 CLOSED

*Reproduced*: `git diff --stat e283bed77 8c712657f -- . ':!lean_frontend/docs'` = the same five files as before (`26 insertions(+), 8 deletions(-)`) — the delta is docs-only; `git show 8c712657f` = `lean_frontend/docs/2026-09-20_program-data-parameters-S1.5-record.md | 22 +`, appended after §9, headed `## Erratum [AGENT 2026-09-20] — the `check_fuel_forms: OK` line quoted in §7.2 was a VACUOUS verdict`. **Accuracy against M1 and the hotfix record §0:** it names the quoted line verbatim, the nine obligations (*"the six `CerbMem` rows of `scripts/fuel_hypotheses.txt:60-65` and the three hand-written seam obligations"*), the mechanism (the module *"had not compiled since seam-hygiene H1 (`fce1de9f8`, 2026-09-19), no step of row 1 rebuilt it (a Lake root imported by nothing), and `scripts/check_fuel_forms.sh` imported its pre-H1 `.olean` as found"*), the finder and the repair (*"F-1, found by the E-A Phase 1 worker on 2026-09-20 and repaired by the hotfix `fix/fuel-forms-carriers` (record … §0: the gate now builds every module it imports, plant P24), which lands immediately after this range"*), what stands (*"Every OTHER verdict quoted in §7 stands: the pin bump moved no source"*), and its provenance (*"this paragraph is that audit's fix, committed on `fix/s15-record-erratum` from `52af8ccf1` so that it lands WITH the range"*) — all consistent with M1, §2.3 above and the hotfix record §0 (which lists "S1.5 Part B" among the vacuous runs). It does not rewrite §7. **One cite slip (NOTE N8, no fix required):** *"the byte-identity witness (§5) is independent of the gate"* — the byte-identity witness is **§6** (B3); §5 is the fork-drift manifest row (*reproduced*: the record's headings `## 5. B2 — the fork-drift manifest …`, `## 6. B3 — byte identity …`). The sentence's claim is true of §6.

### 6.2 Range 3 — the rebase `5a5579209 → 5d462da86`

*Reproduced*: `git range-diff e283bed77..5a5579209 8c712657f..5d462da86` →
```
...
```
(both `=`: identical patch text); `git diff --stat 5a5579209 5d462da86 -- <the hotfix's nine files>` → EMPTY; `git diff --stat 5a5579209 5d462da86` = exactly the S1.5 range's eight files (the three docs incl. the erratum-bearing record, the four pin files, the manifest) — the rebase added nothing of its own. `b89a010ab^` = `8c712657f`, `5d462da86^` = `b89a010ab`. **Everything audited in §2.4–§2.10 carries over to `5d462da86` unchanged.**

### 6.3 Range 3 — the fix commit `e5065fc5a` (7 files: `CLAUDE.md | 4`, seam-hygiene record `| 2`, the hotfix record `| 287`, `scripts/common.sh | 16`, `failure_reach_register.txt | 2`, `fork_drift_manifest.txt | 13`, `test_unit.sh | 3`)

- **M2 → option (a) taken.** `scripts/common.sh` `build_lean`: ONE functional hunk, `lake build cerberus-lean` → `lake build CerberusLean cerberus-lean`, plus a 14-line comment naming F-1, `CerbConcurrency`, `Cabs_to_ail_auxiliary`, the measurement and the fail-by-name rule (*reproduced* by reading the diff; nothing else in the file moves). `scripts/fork_drift_manifest.txt`: an 11-line dated NOTE (`# NOTE (hotfix fix/fuel-forms-carriers, 2026-09-20; … pre-merge audit M2 ruling [AGENT orchestrator]): the [source-content] row for scripts/common.sh is RE-PINNED …`) + exactly one non-comment row change `-100755 77f5ab3a…` / `+100755 008b5ade8b923f9d7ae0fe6fce837dee3e7a75ef2c414dde8791a78633545744 scripts/common.sh` (*reproduced*: the diff's non-`#` lines are those two; the file's first line is still the S1.5 NOTE — no `--refresh`). **The new sha256 matches the committed file:** `git show e5065fc5a:scripts/common.sh | sha256sum` → `008b5ade8b923f9d7ae0fe6fce837dee3e7a75ef2c414dde8791a78633545744`; `git ls-tree e5065fc5a scripts/common.sh` → mode `100755` (*reproduced*). The two named roots: `git grep -l 'import CerbConcurrency' e5065fc5a -- lean_frontend` → none; `git grep -l 'import Cabs_to_ail_auxiliary' e5065fc5a -- lean_frontend` → none, and `lean_frontend/generated/Cabs_to_ail_auxiliary.lean` exists (a carrier the hardened gate builds) — both claims true (*reproduced*). **Every root builds:** the worker's `.tmp/ffc/build-rebased.log` (read-only) → `Build completed successfully (395 jobs).`, `Built`=277, `Replayed`=0 (the LemLib pin move rebuilt everything) — as §8.3 says; MY run of the new `build_lean` itself at `e5065fc5a` (`.tmp/audit/delta/build_lean.log`): `scripts/ce bash -c 'source scripts/common.sh && build_lean'` → `build_lean rc=0 wall=171s` (first pass; the checkout moved my LemLib pin too, and the box load was 48 from another agent's job), `Build completed successfully (395 jobs).`, `check_driver_fresh: recorded lean stamp (bin 688cf879063e124660783b307d3565bbc72f13862b5b019a07741a9fd8883f93, src 809ad92c6258415628ce87897cdc185da6caea6b4e058…)`; the second run `build_lean rc=0 wall=1s` — steady state 1 s, as claimed.
- **M3 → §5 rewritten to the committed text.** *Reproduced*: each of the SIX blockquotes of the new §5 ("as COMMITTED") is a literal substring of the committed file at `e5065fc5a` — quotes 1–2 of `VALIDATION.md` (unchanged in the fix commit: `git diff --stat 5d462da86 e5065fc5a -- lean_frontend/VALIDATION.md` → empty), quotes 3–6 of `lean_frontend/CLAUDE.md` (`quotes=6 found=6`). The two `CLAUDE.md` rows the fix commit edits (`CerbMem_lemMeasureProofs.lean`: *"built by `build_lean` (every root, 2026-09-20) and by the fuel-forms gate itself"*; `scripts/common.sh`: *"`build_lean` builds EVERY Lake root + the exe (…)"*) are now TRUE of the code (option (a) is in `common.sh`); the seam-hygiene §12 sentence gains *"`build_lean` builds every root"* (one line; the erratum's substance unchanged). **Closed.**
- **N1 → head-frame cites.** The register's new row: `CerbMem.lean:2398-2404` → `CerbMem.lean:2428-2434: `size` :2429, the reconstructValue call :2434; def loadM :2419` — the head numbers I measured in N1; the change is inside the `need` field only (the diff's one row differs only there; the seal `21fa14337db08be1` is unchanged — `need` is outside `sha256(file|definition|token|msg|scope|position|position_reviewed|reach)`), and the gate at `e5065fc5a` reads `check_failure_reach: OK (234 pure failure sites = the 234 register rows exactly (232 in the exec dependency closure + 2 unresolved-owner; …); position classes unchanged; 0 DISCARDABLE; … every row sealed; tally line consistent)` (*reproduced*, inside my row-1 run below). The record §3.4/§3.6 cites likewise carry the head numbers with the base numbers noted. **Closed.**
- **N3 → consumer note.** New §7 *"Consumer note (cerberus-sl) — audit N3"*: names the two arms' DEFINITION change, the six `Repr.lean` unfoldings (integer/pointer arms), "nothing breaks at their pin; it is a re-pin-visible change", and the twin — matches my N3; §3.5 (d) reworded (*"the DEFINITION of the two arms changes (a re-pin-visible change for any consumer that unfolds them — audit N3, §7), behaviour on the defined domain does not"*). **Closed.**
- **N6 → `test_unit.sh:224`** now *"(25 plants incl. the whole-project audit's two decoys verbatim and the F-1 stale-carrier plant P24, 2026-09-20)"* — a comment; the invocation is unchanged. **Closed.**
- **N2** recorded as a follow-up in §6 (a type-punning witness) — as I recommended; the row stays `UNKNOWN`. **Accepted.**
- **The record's §8** (rebase, fix, re-gate): §8.1's `range-diff` lines = mine; §8.2's list = the diff; §8.3's `277 targets Built, 0 Replayed` = the log; §8.4's first-run `Source unchanged: False` explanation — *reproduced* (read-only): `.tmp/ffc/release-fast-rebased/summary.txt` = `fast: incomplete; 16/16 selected commands completed successfully.` / `Source unchanged: False. …`; the FINAL run `.tmp/ffc/release-fast-rebased-final/summary.txt` = `fast: passed; 16/16 selected commands completed successfully.` / `Source unchanged: True. Complete tier selection: True.`, the 16 `PASSED` lines with the quoted timings, and `A1/stdout:733-737` `check_fork_content: OK — 76 source files content/mode-pinned` / `check_fork_drift: OK — … lem-pin 38f87d5 = lem -v` / `check_fork_drift: SELFTEST OK (14 plants …)`, `:483 Total: 12 passed, 0 failed`, `:697/:699` the fuel-forms SELFTEST OK/OK lines, `:712` failure-reach OK, `:738` fixture-freeze OK, `:751` renumber-plants OK. **Every quoted line resolves.** The header paragraph's account of my audit (M2/M3/N1/N2/N3/N6, commit `3d614bcda`) is accurate.

### 6.4 Row 1 at `e5065fc5a` — reproduced in my worktree

`git checkout --detach e5065fc5a`; `scripts/ce make lean-prelude-src` (`check_handwritten_sync: OK (49 …)`, `check_lem_sync: recorded lean_frontend/lem_sync.sha256 (src 037dee26…, gen cd499eab…)`); `build_lean` ×2 as above; then `scripts/ce ./scripts/test_unit.sh` (`.tmp/audit/delta/test_unit.log`, 19:54:36 → 19:58:16): **`test_unit EXIT=0`** — `Total: 12 passed, 0 failed`; `check_exec_purity: CLEAN (11 modules)`; `check_theorem_axioms: OK (…)`; `check_sorry_token: OK (318 files …)`; `check_no_fuel_numerals: OK (325 files …)`; `check_lakefile_roots: OK (218 roots = 218 generated modules + the exe root Main; 85 auxiliary modules listed as roots — names only; every carrier is built by check_fuel_forms.sh)`; `check_fuel_forms: SELFTEST OK (25 plants …)` (P24 `PLANT OK` among them) / `check_fuel_forms: OK (81 fuel'd workers: 62 MEASURED (…; 12 of them under a hypothesis, …)`; `check_failure_reach: OK (234 …)`; `check_exec_totality: CLEAN (22 generated modules + hand-written CerbND, 0 allowlisted)`; `check_lem_sync: OK (src 037dee26…, gen 08b84774…)` / `check_lem_sync: lean OK (src 037dee26…, gen cd499eab…)`; `check_fork_drift: SELFTEST OK (14 plants with declared verdict/message: …)` (S1–S14 all `PLANT OK`; S9 now plants against `lem-pin=38f87d5`) / `check_fork_content: OK — 76 source files content/mode-pinned` / `check_fork_drift: OK — layer 1: 76 oracle-surface files = manifest (set, C-locale canonical, no duplicates); layer 2: 25 differing generated files, all hash-pinned (merge-base b9aeedcb4dd438763b0eef7f95ac19e93875d7de; lem-pin 38f87d5 = lem -v)`; `check_fixture_freeze: OK (16 fixture files match the pinned manifest; name set exact)`; `test_renumber_plants: OK (12 plants: refusals refuse, admits admit with declared class)`. The environment red line of §2.9 is GONE — the rebased manifest's `lem-pin=38f87d5` = the switch's `lem -v`, and the re-pinned `common.sh` row passes the content layer. **Row 1 fully green, reproduced.** Rows 2–12 at `e5065fc5a` I did not re-run: the fix commit touches no semantics, seam, generated, baseline or pin file (`common.sh`'s one hunk changes what `build_lean` BUILDS, not what any lane runs), the rebased hotfix commit is `=` to the one whose rows 2–12 I re-ran (§2.9), and the worker's final run's 15 lane tails (read-only) are identical to those I measured in §2.9 — accepted on those grounds.

### 6.5 Final verdicts

- **lem-lean `4307dc5..38f87d5`: MERGEABLE** (unchanged; nothing in either delta touches it).
- **cerberus `e283bed77..8c712657f`: MERGEABLE.** M1 closed by `8c712657f` (verified accurate; one section-number slip, N8, NOTE only). Remaining NOTEs N4/N5 need no action.
- **cerberus `8c712657f..e5065fc5a`: MERGEABLE.** The rebase is exact (`range-diff` `=` ×2, the nine files byte-identical); M2 closed by option (a) with the `common.sh` content pin re-pinned as a single row + NOTE (sha256 verified against the committed blob; every root builds — the worker's 277-target build and my own `build_lean` runs); M3 closed (§5's six quotes are literal substrings of the committed docs); N1/N3/N6 closed; N2 recorded; row 1 fully green in my worktree at `e5065fc5a`; the worker's 16/16 re-gate verified verbatim. Remaining NOTEs (N2 as a follow-up, N5, N6 (i)/(iii)) need no action before merge.

**Landing premise re-confirmed:** range 2 then range 3 in that order; range 3 is already ON range 2's head (`b89a010ab^ = 8c712657f`), so the merge of range 3 is ff-only after range 2's.

**Not checked in the delta:** rows 2–12 at `e5065fc5a` (accepted, §6.4); the worker's first rebased run beyond its summary; whether cerberus-sl will act on §7 (theirs).

[AGENT auditor] 2026-09-20 ≈ 19:20–20:00 UTC; scratch under `.tmp/audit/delta/` (git-ignored, ephemeral); the hotfix audit worktree was checked out detached at `e5065fc5a` for §6.4 and is back on `audit/fuel-forms-carriers`; the S1.5 audit worktree and the worker/fix worktrees were not modified; nothing pushed; no commit on any mainline or worker branch.
