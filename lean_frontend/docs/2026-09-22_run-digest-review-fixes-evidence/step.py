...
limit = int(limit)
root = Path('.tmp/run-digest-review')
...
while True:
    ...
    for line in subprocess.check_output(['ps', '-eo', 'pid=,comm=,args='], text=True).splitlines():
        ...
        if (int(pid) == 3187904 and '.tmp/orch/run-digest-gates.log' in args) or comm in ('lean', 'lem') or (comm == 'lake' and ' build' in args) or (comm == 'dune' and ' build' in args) or (comm.startswith('python') and 'scripts/release.py' in args):
...
with (root/(name+'.log')).open('w') as log:
    ...
    try:
        while True:
            ...
            if name == 'fast' and coordination.exists():
