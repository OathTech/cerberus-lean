...
explicitly recorded waits from its active-time budget.
"""
    ...
    while True:
        ...
        for pid, (_, status, comm, command) in procs.items():
            ...
            heavy = (comm in ('lean', 'lem') or
                     ...
                     (comm.startswith('python') and any(name in command for name in
                         ('scripts/release.py', 'scripts/test_upstream_oracle.py'))) or
                     comm == 'cerberus-lean')
            if heavy:
