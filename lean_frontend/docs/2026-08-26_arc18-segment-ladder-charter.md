...
R0 EXECUTED 2026-08-26: pre-merge audit MERGE-SAFE (zero MAJOR, 4
MINOR fix-forward — see docs/2026-08-26_arc18-r0-premerge-audit.md
and the fix-forward register at the end of this file); C0–C4
ff-merged to `mdd/cerberus-lean` @ cfeb7af5e on [USER] sign-off
("land these first").
...
## Fix-forward register (from the R0 pre-merge audit; movers named)

- **FF-1 (probe recipe, mover: R3 or arc-19 tooling):** `lake lean` on
  files importing the C1-decomposed engine modules
  (RelSem.RoundEval.Core closure) mis-resolves to the ROOT package's
  build tree and fails; `lake build` + all gates unaffected. The
  documented probe recipe is false for the new engine modules; root
  `.lake` also carries pre-slice orphan artifacts (arc-11 split era)
  due for deletion per the stale-shadow doctrine. Diagnose Lake
  attribution, purge orphans, re-verify the recipe before arc-19
  leans on it.
- **FF-2 (bisectability, note only):** two intermediate C3b/C4 commits
  are not self-contained standalone builds (disclosed in their
  messages); HEAD fully validated. No action; noted for any future
  bisect.
- **FF-3 (engine-size watch, mover: next re-baseline point ≤R3):**
  baseline 5207 (C3) vs HEAD 5680 (+473 across C3b/C4, 9 standing
  WARN lines); re-baseline with provenance or shrink, and record the
  growth (down-pressure register row is the enforcement locus).
- **FF-4 (PROOF.md §3 currency, mover: R2-close doc touch):** §3
  stale re T4 ("scheduled to complete... arc-18 C3" — did not; R5
  OPEN) and the landed T5 walk/family substrate; refresh at the R2
  interim report at latest (capability-changing merges re-read §3
  per doctrine).
- Audit observations (wording/cite nits incl. the "kernel-witnessed
  FALSE" overstatement vs the acceptance doc's careful reading):
  fold into the same R2-close doc touch.
