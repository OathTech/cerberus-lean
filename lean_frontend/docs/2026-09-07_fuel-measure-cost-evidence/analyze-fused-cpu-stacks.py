...
D3 final variant: also identify the specialized well-founded context bound.
The generated Core_reduction.c confirms this specialized function computes
only the measure. Work inlined into the wrapper itself cannot be separated.
...
def analyze(binary, trace):
    ...
    for start in range(24, len(data), 98 * 8):
        ...
        def search_bound(s):
            return "CerbTagsWf_callBound" in s or bool(re.search(
                r"WellFounded_fixC.*___at___.*get__ctx_spec__", s))
        search = any(search_bound(s) for s in stack)
        env = any("CerbTagsWf_" in s and not search_bound(s) and re.search(
    ...
    for symbol, count in inclusive.most_common():
        if search_bound(symbol) or "lemSize" in symbol or re.search(r"all\w*Bound", symbol) or (
