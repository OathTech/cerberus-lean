...
Usage: python3 analyze-cpu-stacks.py BINARY TRACE [TRACE...]
...
def analyze(binary, trace):
    ...
    for start in range(24, len(data), 98 * 8):
        ...
        env = any("CerbTagsWf_" in s and re.search(
        ...
        for category, present in (("environment measures", env),
                                  ("generic expression measures", expr),
                                  ("ctype measures", ctype),
                                  ("all measures union", env or expr or ctype or other),
                                  ("rest", not (env or expr or ctype or other))):
