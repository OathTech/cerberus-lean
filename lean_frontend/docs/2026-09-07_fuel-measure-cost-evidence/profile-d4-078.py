#!/usr/bin/env python3
"""Profile the D4 CPU-ratio exception with the checked D2 CPU sampler.
...
def main():
    ...
    out = root / '.tmp/fuel-measure-cost/d4-profile-078'
    ...
    for repeat in (1, 2):
        for rev in ('pre', 'after') if repeat == 1 else ('after', 'pre'):
            ...
            command = ['/usr/bin/time', '-v', '-o', str(prefix) + '.time', 'timeout', '90',
                       ...
                       f'FC_SAMPLE_FILE={prefix}.samples', 'FC_SAMPLE_US=1000', str(binary), '--batch', str(bridge)]
