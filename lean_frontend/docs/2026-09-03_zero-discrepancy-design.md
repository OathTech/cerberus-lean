...
Slice Z1 (2026-09-03, record `docs/2026-09-03_zero-discrepancy-Z1-record.md`)
changed the class of the rows it owned — Z-01/02/03/05/06/07/08/10/72/76
FIXED (+ Z-09/11/12 in the same hunks), Z-24/25 refusal LANDED (EXC(c)),
Z-27 CLOSED (EXC(c) with the op-by-op table), Z-73 DECLARED, Z-74
MEASURED, Z-36 pinned (register R1), Z-67/68/69 seam-index rows resolved
for Z1's part — each in its row below, with the commit; and appended
§2.4b (three Z1 additions). The §0 totals above are the census AT BIRTH
and are not re-derived here (Z4 re-derives them with its re-sweep).
...
|---|---|---|---|---|---|
| Z-01 (D1) | noodle §0 D1; `float/float_inf_to_int_ub.c` | `Undefined {ub: "UB017_out_of_range_floating_integer_conversion", stderr: "", loc: "<5:11--5:19>"}` | `… loc: "unknown location"` — every UB raised while executing std.core code (UB017, UB153a/b, UB158, Invalid_format, the 47 `undef(<<DUMMY>>)` sites) loses the C site | **FIXED — Z1 `1c1311a57`** (docs/2026-09-03_zero-discrepancy-Z1-record.md §2; loc now `<5:11--5:19>` = oracle; the Pos line/col are a documented divergence, file exact) — was BUG-FIX (§1.3) **Z2 Pos row (`5ed6c4a0a`)**: the `PEundef`/`Action` positions of library files are now the OCaml's exact line/column (`CoreParser.markerLoc`/`stampLibraryFile`); the other nodes stay file-only, declared (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.7) | `CoreParser.lean:201/204/1097` stamp std.core nodes with a `Loc.region` whose file is the std.core path so `CerbLocation.isLibraryLocation` holds (mirror `core_parser.mly:1571`); then the shared `core_eval.lem:596-603` / `core_run.lem:778-784` substitution works as on OCaml. **S** |
| Z-02 (D2) | noodle §2 D2; `ptr/ptr_to_int_narrow_ub.c` | `Undefined {ub: "UB024_out_of_range_pointer_to_integer_conversion", stderr: "", loc: "<7:11--7:17>"}` | `… loc: "other_location(Concrete)"` | **FIXED — Z1 `1c1311a57`** (docs/2026-09-03_zero-discrepancy-Z1-record.md §2) — was BUG-FIX (§1.3) | `CerbMem.lean:2297` `memFail (MerrIntFromPtr)` → `memFail MerrIntFromPtr loc` (mirror `impl_mem.ml:2459` `fail ~loc`). **S** |
| Z-03 (O1) | noodle §0 O1; this charter's probe P-C3 | `loc: "<6:10--6:12>"` (`Cerb_location.simple_location`) | `loc: ".zd-scratch/probes/pnvi.c:6:10-12"` (`CerbLocation.stringFromLocation`) | **FIXED — Z1 `1c1311a57`** (`CerbLocation.simpleLocation`; docs/2026-09-03_zero-discrepancy-Z1-record.md §2) — was BUG-FIX — the `loc` field is behaviour (§1.3); its RENDERING must agree byte-wise or no lane can compare it. Cheaper and doctrinally right to mirror the printer than to normalise in every harness | `CerbLocation.lean` batch renderer mirrors `simple_location` (`util/cerb_location.ml`), cite. **S** (prerequisite of §4.1) |
...
|---|---|---|---|---|---|
| Z-09 | `CerbMem.lean:2282` vs `impl_mem.ml:2163-2173` | `ptrfromint` maps `n = 0` to `PVnull` only when no provenance is carried (per the OCaml arm structure) | `if n == 0 then PVnull` regardless of `prov` | **FIXED — Z1 `c61b78f70`** (same hunk as Z-06: the `ptrfromint` PVI arm structure of impl_mem.ml:2163-2173; docs/2026-09-03_zero-discrepancy-Z1-record.md §2) — was BUG-FIX — unobservable today ONLY because P2 (Z-40) strips provenance on every arithmetic path; the moment upstream fixes P2 and Lean mirrors it, this becomes value-visible. Fix now, before it can bite | mirror the arm. **S** |
| Z-10 (R2) | `CerbMem.lean:1906-1909` vs `impl_mem.ml:1527-1534` (`:1532` `failwith "Concrete: FREE was called on a dead allocation"`) | dead-allocation arm: `failwith` (uncaught exception, exit 125) for a NON-dynamic kill of a dead allocation — witnessed: `core_bug_then_kill.core` on both oracles (dynamic-addrs row k) | `UB179b_dead_allocation_free` regardless of `isDynamic` — witnessed: `inj_bug.c` + `inject_rand.core` → `Undefined {ub: "UB179b_dead_allocation_free", stderr: "", loc: "tests/noodle-probes/dynamic-addrs/inj_bug.c:10:3-12"}` (row n') | **FIXED — Z1 `c61b78f70`** (`panic! "Concrete: FREE was called on a dead allocation"` with the in-code reachability note; witness inj_bug.c → PANIC exit 134; docs/2026-09-03_zero-discrepancy-Z1-record.md §2) — was BUG-FIX — a one-sided oracle crash; under Q4's recommendation Lean mirrors it as a fail-stop with the OCaml text (a failure-class difference, not (a)). Reachability: only after an ACCEPTED wrong `free` — i.e. only through the tray-19 `dynamic_addrs` defect (Z-77) or the fork-only `cerb::with_address` — say so in-code. The kill-check ORDER difference the same investigation recorded is Z-07/Z-08 (noodle D6/D7), not a new row | mirror as a fail-stop (`panic!` with the OCaml text). **S** |
| Z-11 | `CerbMem.lean:1909` vs `impl_mem.ml:669-675` (`get_allocation`) | missing allocation → UB009 (via `get_allocation`) | UB179a | **FIXED — Z1 `c61b78f70`** (same hunk as Z-08: `get_allocation` → MerrOutsideLifetime/UB009; docs/2026-09-03_zero-discrepancy-Z1-record.md §2) — was BUG-FIX (UB code) | mirror. **S** |
| Z-12 | `CerbMem.lean:1901-1903` vs `impl_mem.ml:1465-1469` | non-dynamic kill of NULL succeeds | fails (UB179a) | **FIXED — Z1 `c61b78f70`** (same hunk as Z-07: the null arm is not conditional on is_dyn; docs/2026-09-03_zero-discrepancy-Z1-record.md §2) — was BUG-FIX (verdict class) | mirror. **S** |
| Z-13 (R1) | `CerbMem.lean:1849-1850`, `:1877-1878` vs `impl_mem.ml:1290`, `:1247-1258` | no clamp: size/align 0 flow through; negative sizes are the OCaml's own arithmetic | TWO distinct silent normalisations: `allocateObject` clamps BOTH size and align with `.max 1` (`:1849-1850`); `allocateRegion` clamps only align (`:1877`) and `size := sizeN.toNat` (`:1878`) silently maps a NEGATIVE size to 0 | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** — unreachable today (`int a[0]` rejected by the shared front end, probe S8) but a silent normalisation with no cite; mirror or declare with the reachability argument in-code | mirror (delete the clamps, keep the OCaml arithmetic). **S** |
| Z-14 | `CerbMem.lean:1845` vs `impl_mem.ml:1291-1297` | `req_addr_opt = Some _` → `failwith` | argument ignored | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** (silent absorption of a fail-stop) | mirror the fail-stop. **S** |
| Z-15 | `CerbMem.lean:1884` vs `impl_mem.ml:1428-1429` | `allocate_region` records `PrefMalloc` unconditionally | stores the caller's `pref` | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** — the prefix is printed by the mirrored `pp_prefix` (`CerbPP.stringFromSymbol_prefix`) on pointer-value renderings; observable wherever a malloc'd pointer value is printed with its prefix | mirror. **S** |
| Z-16 | `CerbMem.lean:591-602/504-510` vs `impl_mem.ml:1096-1113` | `bytes_of_int` asserts on out-of-range | wraps silently | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** (silent absorption of a fail-stop); unreachable today (`conv_int` precedes every store) — mirror the assert anyway | mirror. **S** |
| Z-17 | `CerbMem.lean:2303-2304` vs `impl_mem.ml:2244-2356` | `eff_array_shift_ptrval`: null → UB046; GNU void-element arm; union-member tag dropped | delegates to the pure shift: null → `panic!` (`:1506-1507`); void case differs; tag kept | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** — reachable only under strict/PNVI/CHERI (`translation.lem:2112-2119`), all of which Lean will REFUSE (Z-24/25); still a panic where the oracle gives UB046 — mirror rather than carry a dead panic | port `eff_array_shift_ptrval`. **S** |
| Z-18 | `CerbMem.lean:951` vs `impl_mem.ml:986-994` | `reconstruct` on zero-sized-element arrays takes the OCaml path | short-circuits | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** (mirror or declare; unreachable with the front end's zero-size rejection) | mirror. **S** |
| Z-19 | `CerbMem.lean:931` vs `impl_mem.ml:1056-1057` | unspecified pointer value's ctype drops pointee qualifiers | keeps them | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** — `Unspecified('<ctype>')` is a VERDICT VALUE (batch prints it); a qualifier in the ctype text is a visible value difference on any unspecified-pointer read | mirror. **S** |
| Z-20 | `CerbMem.lean:2402-2403` vs `impl_mem.ml:2683` | `realloc` `get_allocation` failure carries `other "Concrete.realloc"` loc | passes no loc (its own comment quotes the OCaml loc) | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** (§1.3 — loc is behaviour) | pass the loc. **S** |
| Z-21 | `CerbMem.lean:2225-2228` vs `impl_mem.ml:2067-2070` | one `MerrOther` message | two messages + a `FunctionNoParams` arm | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **EXC(a)** — same failure class (`Error`), text differs. Mirror anyway in Z2 (trivial) so the census row retires | mirror. **S** |
| Z-22 | `CerbMem.lean:1759-1764`, `:2548` vs `impl_mem.ml:2175-2191` | CHERI intrinsics: `assert false` | return values | **FIXED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6) — was **BUG-FIX** — a value where the oracle fail-stops (silent absorption); unreachable once CHERI is refused (Z-24) but a dead fail-open path is still the banned shape | mirror as fail-stop. **S** |
| Z-23 | `CerbMem.lean:2425, 563, 2175, 2389, 2344, 2311` (stale cites → `impl_mem.ml:1704-1710/1776-1787, 1202, 1954-2063, 2679, 2664-2666, 2635-2645`) | — | — | **RE-CITED — Z2 `3744e8503`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6; five of the six sites were already correct in-tree) — was **INSTRUMENT** (doc integrity; no execution content) | re-cite in Z2. **S** |

### 2.3 VALIDATION.md's declared boundaries, re-classified

| id | boundary | oracle | Lean (today, verified) | class | fix + price |
|---|---|---|---|---|---|
| Z-24 | **PVI vs PNVI** (VALIDATION.md §5 does not name it; `CerbGlobal.lean:is_PNVI_impl := false`, `has_strict_pointer_arith_impl := false`, `switchesRef` permanently `[]` — no setter exists) | `--switches=PNVI` CHANGES the answer: probe P-C1 default `Undefined {ub: "UB043_indirection_invalid_value", stderr: "", loc: "<6:10--6:12>"}`; P-C2 with `--switches=PNVI` `Defined {value: "Specified(20)", stdout: "", stderr: "", blocked: "false"}` | matched (default) mode agrees: P-C3 `Undefined {ub: "UB043_indirection_invalid_value", stderr: "", loc: ".zd-scratch/probes/pnvi.c:6:10-12"}`. The FLAG is not refused: `Main.lean:1024-1027` treats any unknown token as a file name → P-A1 `uncaught exception: no such file or directory (error code: 4294967294)` / `file: --switches=PNVI`, rc 1 | **EXC(c) — REFUSAL LANDED, Z1 `b42776130`** (`Main.refuseFlag`, attributed, exit 2; plants in docs/2026-09-03_zero-discrepancy-Z1-record.md §2) — was BUG-FIX → then EXC(c). Loud but NOT feature-attributed (§1.2(c)); a harness that ever passed a switch would get a misleading error, and a MISORDERED legitimate flag is a file name (`Main.lean:980-982` admits it; R1 probe, verbatim: `cerberus-lean --batch at.json --first` → `uncaught exception: no such file or directory (error code: 4294967294)` / `file: --first`, rc 1). Under the rule a fail-open CLI is a defect; the refusal must also reject KNOWN flags out of canonical position | `Main.lean` arg loop: any `--`-prefixed token not in the accepted set → `cerberus-lean: refused — <flag>: semantics switches / concurrency / <x> are not supported by this port (VALIDATION.md §<census>)`, exit 2. Plant: the P-A probes. **S** (plumbing the switches for real is M and NOT proposed — mode parity is the harness contract) |
| Z-25 | **Concurrency stubs** (VALIDATION.md §5 "temporal, the cmm instantiation is the mover"; `CerbConcurrency.lean` `statically_satisfied := true`; `CerbGlobal.using_concurrency := false`; TODO.md "Concurrency (cmm) instantiation") | `--concurrency` on ANY program: probe P-B3 `internal error: CONCURRENCY IS BROKEN` … `Failure("internal error: CONCURRENCY IS BROKEN")` (raised from `nondeterminism.ml:64` via `smt2.ml:38`) — the oracle has no working concurrency mode; the shared `.lem` hard-codes `perform_action_request2 false (*TODO!!! with_concurrency*)` (`driver.lem:980`). Matched (default) mode: atomics run sequentially and AGREE (noodle `elab/elab_atomic_qualifier_seq.c` "AGREE 3-way 8"; R1 probe `_Atomic int a = 5; a += 2; a++; return a;` — oracle `Defined {value: "Specified(8)", stdout: "", stderr: "", blocked: "false"}`, Lean identical, gcc exit 8; the earlier `atomic_fetch_add`-under-`--nolibc` probe was vacuous (both-reject on an undeclared identifier) and is withdrawn) | a program reaching the stubs in matched mode computes what the oracle computes (`Global.using_concurrency ()` gates only the `core_run_aux.lem:342/411/429/494` bookkeeping, false on both sides). The FLAG is not refused: P-A2 `uncaught exception: no such file or directory` / `file: --concurrency` | **EXC(c) — Z-24's refusal landed (Z1 `b42776130`)**; VALIDATION.md says "concurrency: not supported; the oracle's own mode is non-functional at b9aeedcb4" — no silent computation exists: with the flag the oracle itself fail-stops, without it both engines agree. The stub is therefore NOT a BUG row; the refusal quality is (Z-24). VALIDATION.md must say "concurrency: not supported; the oracle's own mode is non-functional at `b9aeedcb4`". Whether the cmm instantiation mover becomes OPTIONAL is a PRODUCT-SCOPE change → operator ASK, §7 Q9 (R1) | rides Z-24. **S** |
...
| Z-36 | noodle E2 `ptr/ptr_string_literals.c` (`"\?"` in a STRING literal) | exit 125 `Failure("decode_character_constant, started like an octal constant, but failed: ?")` (`translation.ml:3032`) | `98 65 66 4 83 52 3 10 9 92 34 39 63 0 4` = gcc byte-for-byte | **REGISTER R1 — pinned by Z1** (`tests/immaculate/nolibc/zd-e2-ptr-string-literals.c` ORACLE_CRASH / L=Specified(0); VALIDATION.md "ISO-fix register") — was REG-CAND R1 (same decoder as Z-33; one register entry covers both) + tray 10 addendum; pin as immaculate `ORACLE_CRASH` pair | as Z-33 |
...
### 2.4b Z1 additions (2026-09-03, slice Z1; evidence in `docs/2026-09-03_zero-discrepancy-Z1-record.md` §7)

| id | cite | oracle | Lean | class | fix + price |
|---|---|---|---|---|---|
| Z1-A1 | Z1 record §7.1; `tests/suite/fs/fprintf_then_fscanf.c` (libc) | UB048 with `loc: "<116:23--116:30>"` — `runtime/libc/src/stdio.c:116`, a UB raised INSIDE a libc C body; the oracle's libc.co carries the libc source locations | `loc: "<unknown location>"` — the `--libc` pin is the oracle's Core TEXT dump (`tests/libc/libc.core`), which has no locations; the libc bodies are not library-located on either side, so `current_loc` is overwritten with `unknown` while libc code runs | **BUG-FIX** (§1.3: UB loc is behaviour) with a named mover — a libc pin vehicle that carries locations (a `--pp=core` variant emitting `Aloc`s, or a Lean reader of the marshalled `.co`); surfaces as `UB_DIFF` rows in Z4's `test_ci_sweep.sh` re-record now that the sweep compares whole Undefined lines (also `s4b-memcmp-hugesize`'s Lean token) | libc pin vehicle. **S–M**, Z4 or a follow-up |
| Z1-A2 | Z1 record §7.2; `scripts/test_exec.sh` exit logic | — | — | **INSTRUMENT** (pre-existing): default (non-baseline) mode does not count `UB_DIFF` toward the exit code (only MISMATCH/FAIL/CRASH/FUEL/ERROR/TIMEOUT/HANG) — the recorded-DIFF design; every gate row runs `--check-baseline`, where UB_MATCH → UB_DIFF is a fatal REGRESSION (plant (c′) in the record). Say so in the Z4 VALIDATION rewrite; consider a `cerb_skip` ceiling too (record §7.6) | Z4, **S** |
| Z1-A3 | Z1 record §7.8; `lean_frontend/speclab/SpecLab/ListAppendCore.lean` | — | the derived (emitter-generated) pinned module had `alloc((16 : Int), size)` from the pre-Z-76 CoreParser | **INSTRUMENT — FIXED** (`8442a67d8`, regenerated; the drift gate caught it) | done |

### 2.4c Z2 additions (2026-09-03/04, slice Z2; evidence in `docs/2026-09-04_zero-discrepancy-Z2-record.md` §1/§2; the audit `docs/2026-09-03_zero-discrepancy-Z2-audit.md` §2 lists every candidate row — all disposed in code, `grep "zero-discrepancy Z2-" lean_frontend/*.lean`)

| id | cite | oracle | Lean (before) | class | fix + price |
|---|---|---|---|---|---|
| Z2-M-01 | audit §2.1.1; `tests/z2-probes/mem/aligned_alloc_zero*.c` | uncaught `Division_by_zero` at `Concrete.op_ival` (impl_mem.ml:2482) via `std.core:385` `size rem_t align` — fork AND upstream, exit 125 | `Undefined {ub: "DUMMY(align_alloc)", …}` / `Defined {value: "Specified(1)", …}` (total `Int.tmod`) | **PENDING — operator decision** ([USER 2026-09-03] logical-semantics referent ruling, `docs/2026-09-03_logical-semantics-referent-ruling.md`: the oracle's `Division_by_zero` is a KIND-2 OCaml-execution artifact, NOT mirrored; the fix-group-1 mirror `3744e8503` is REVERTED in `5ed6c4a0a`; Lean keeps the total `rem_t` — pins `zd-z2m01-*` ORACLE_CRASH \| L=UB:DUMMY(align_alloc) ×2 and MATCH \| L=CRASH (the allocator's alignment-0 pending refusal); the logical meaning — Core-level UB045 for division by zero and/or an ISO 7.22.3.1 std.core:385 guard, both shared-model changes — is `docs/2026-09-04_zero-discrepancy-Z2-record.md` §10.1) — was BUG-FIX (Q4) + tray candidate (Z4) | decision |
| Z2-CP-01 | audit §2.14; `coreparser/strtod_inf.c` | 10 × `Specified(1)` (in-memory libc.co holds `OVfloating +inf`) | 10 × `Error {msg: "Unresolved_symbol: …"}` (`inf` lexed as a symbol) | **FIXED — Z2 `3744e8503`** (`inf`/`nan`/`-inf`/`-nan` → OVfloating; binder tripwire; pin `zd-z2cp01-strtod-inf` MATCH) — was BUG-FIX | done |
| Z2-CP-21 (NEW, fix phase) | `pp_core.ml:367-370` vs `core_lexer.mll:83-84`; `tests/libc/libc.core` ×29 | the pp spells `Cfvfromint`/`Civfromfloat`; the lexer keywords are `Fvfromint`/`Ivfromfloat` (the oracle cannot re-read its own dump) | `Error {msg: "Illformed_program: calling an unknown function"}` once `inf` parsed | **FIXED — Z2 `3744e8503`** (both spellings accepted in the ctor arms) — BUG-FIX + SUSPECT→tray (pp/lexer mismatch upstream, Z4) | done |
| Z2-I-01 (+I-02/I-03/I-04) | audit §2.10; `impl/cerbty_int32_uac.c` | `Specified(1)` | `PANIC … AilTypesAux.le_integer_range: internal error` exit 134 (no IntN_t aliasing) | **FIXED — Z2 `3744e8503`** (`CerberusImpl.n_t_aliases`/`aux_ibty`/`normalise_integerType` = ocaml_implementation.ml:37-66/:154-171; sizeof/alignof normalise first; pin `zd-z2i01-cerbty-int32-uac` MATCH) — was BUG-FIX | done |
| Z2-F-01 | audit §2.5; `fs/lseek_whence.c` | `Specified(9)` (SibylFS EINVAL, fs_spec.lem:5084) | `Specified(13)` at the audit; a loud refusal after Z1's Z-27 | **FIXED — Z2 `3744e8503`** (EINVAL for whence ∉ {0,1,2} before any offset work; pin `zd-z2f01-lseek-whence` MATCH) — was BUG-FIX | done |
| Z2-C-01 (= Z-60), Z2-C-02 | audit §2.4; `call/bool_param.c`, `call/errno_order.c` vs the rendered wrapper TUs | `Specified(1)`; `Specified(65524)` | UB012 trap (raw store of 2 into a `_Bool`); `Specified(65528)` (errno after the arguments) | **FIXED — Z2 `3744e8503`** (`CerbCall.mkCallSite` renders translation.lem:940-975/:1126-1155 — the non-variadic Normal_callconv branch; `allocErrno` first; `tests/verify` rows) — was BUG-FIX (harness fidelity) | done |
| Z2-M-03/M-04/M-05/M-07/M-08/M-09/M-10/M-12/M-16, Z2-CP-03/04/07/08/09/11/13/16/18, Z2-P-04/P-05, Z2-L-02, Z2-DC-01, Z2-F-03, Z2-Q-01 | audit §2 (CP-13: the anonymous-tag `a_N`/`__cerbty_unnamed_tag_N` symbol mirror, reached by `test_core.sh tests/ci` — record §2.7/§8.4) | (each row's oracle behaviour, cited in code) | (each row's Lean-before, cited in code) | **MIRRORED — Z2 `3744e8503`/`5ed6c4a0a`/`096d8930e` (CP-13; exact-token tag match in the audit-response commit)** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.6–§2.8; the Z2-P-05 mirror moved the uri lane's pinned `LEAN_NOLIBC` error text to the oracle's wording) | done |
| Z2-M-06/M-11/M-13/M-17/M-20, Z2-N-02/N-03, Z2-P-08, Z2-G-01/G-02, Z2-U-01/U-02, Z2-I-04, Z2-T-01, Z2-Q-02, Z2-D-01, Z2-J-01/J-02, Z2-CP-02/05/06/10/12/15/17/19/20 | audit §2 | — | — | **DECLARED IN CODE — Z2 `3744e8503`/`5ed6c4a0a`** with the reachability argument next to each site; Z2-CP-10/CP-15 and Z2-J-01/J-02 also tray candidates (Z4) | done |
| Z2-FL-01/FL-02, Z2-N-01, Z2-M-14/M-15, Z2-M-19 | audit §2 | — | — | verified-matching / doc integrity (pinned as coverage/libc_exec rows, docs/2026-09-04_zero-discrepancy-Z2-record.md §5) | done |
...
|---|---|---|---|---|
| Z-59 (R1 F3 — evidence corrected, conclusion survives) | `CerbND.lean` header + `:127` ("same no-pruning divergence as NDguard" for `NDbranch`) vs `smt2.ml` `runND` applying `with_constraints`/`check_sat` (`impl_mem.ml:321-361` `cs_module`) | The first draft's evidence was WRONG: `nd_guard` (`nondeterminism.lem:222-223`) produces `kill`, not `NDguard`. `NDguard` is produced by `addConstraints` (`nondeterminism.lem:234-237`, `:499-502`), which IS called in the exec cone: `driver.lem:148` (the `PEconstrained` arm of `print_eval_conv_aux`) and `defacto_memory.lem:513/517` (defacto model, not linked). `PEconstrained` arises only when `Mem.eq_ival`/`lt_ival`/`le_ival` return `Nothing` (`core_eval.lem:352-378`), and the Concrete model ALWAYS returns `Some` (`impl_mem.ml:2556-2562`) — so `NDguard` is unreachable by that argument, not by "no caller". `NDbranch`: produced at `nondeterminism.lem:422` (`msum`, `NDbranch empty …`) with EMPTY constraints (`:465` variant commented out) → `with_constraints` on the oracle side is trivially SAT → exploring both sides is exactly what the oracle does. Noodle: trace COUNTS agree on every completing probe (8/8, 2/2, 40/40, 140/140, 280/280, 67650/67650) | **DISPOSED — Z2 `5ed6c4a0a`** (CerbND header rewritten "unreachable by construction"; the tripwire is the theorems `CerbMem.eqIval_isSome`/`ltIval_isSome`/`leIval_isSome`, `3744e8503`; docs/2026-09-04_zero-discrepancy-Z2-record.md §2.8) — was Z2-DISPOSED: DECLARE with the corrected argument in-code, plus the TRIPWIRE stated as a mirror check — "`CerbMem.eqIval`/`ltIval`/`leIval` (`CerbMem.lean:1434-1439`) are total `some`, mirroring `impl_mem.ml:2556-2562`" (a unit test or `#guard`); if any of them ever returns `none`, mirror `with_constraints` evaluation (**S**) | Z2 |
| Z-60 | `CerbCall.lean` header: "the call-site `conv_int` range conversion is NOT reproduced — an injected integer must fit the parameter type (out-of-range injections are the caller's responsibility)" | a fail-OPEN instrument contract; oracle twin = the rendered wrapper TU (`test_verify.sh render_wrapper`) which DOES convert | **FIXED — Z2 `3744e8503`** (the `--call` entry RENDERS the elaborated call site: conv_loaded_int at the parameter type, errno before the temporaries; `tests/verify/z2_bool_param`, `z2_errno_order`; docs/2026-09-04_zero-discrepancy-Z2-record.md §2.4) — was **BUG-FIX** (instrument fail-closed rule): refuse out-of-range injections loudly (`kill`) or reproduce `conv_int`. **S** | Z2 |
| Z-61 | `CerbPP.lean` "Residual placeholders" (`<...>`-bracketed) | reach no compared text by design ("an honest mismatch, never oracle-shaped") | **VERIFIED — Z2 audit** (every `<…>` placeholder call site feeds an Error/debug string, none a Defined/Undefined field; docs/2026-09-04_zero-discrepancy-Z2-record.md §8.5) — was INSTRUMENT — Z2 verifies by grep that no residual printer is on a batch-verdict path (`Main.lean` batch renderers) | Z2 |
| Z-62 | `CerbFloat.lean` `Ord Float`: "+0.0/-0.0 compare equal here — OCaml's compare distinguishes them" | value-visible only if a float keys a set/map on an exec path | **DISPOSED — Z2 `5ed6c4a0a`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.8) — was Z2-DISPOSED: prove unreachable (no `Set Float` in the exec cone) or mirror `Stdlib.compare`. **S** | Z2 |
| Z-63 | `CerbDecode.lean` `read_digit`: "returns 0 for a non-digit where upstream's read_digit computes garbage" (sem:N11) | garbage-for-garbage; declared unreachable (every caller validates first) | **DISPOSED — Z2 `5ed6c4a0a`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.8) — was Z2-DISPOSED: re-verify the caller set; if reachable, mirror. **S** | Z2 |
| Z-64 | `CerberusFresh.lean` digest in hex form vs OCaml raw 16-byte `Digest.t` | equality-preserving (injective); only `compare = 0` is consumed | **DISPOSED — Z2 `5ed6c4a0a`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.8) — was Z2-DISPOSED: declared and argued in-code; confirm no ORDER on digests reaches an exec path (an order on hex ≠ order on raw bytes) | Z2 |
| Z-65 | `CerberusImpl.lean` LP64 table vs `ocaml_implementation.ml` `DefaultImpl` | mirrors incl. F1's `float = 8` | **DISPOSED — Z2 `5ed6c4a0a`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.8) — was Z2-DISPOSED: diff the two tables entry by entry (sizeof/alignof/`is_signed`/`char` signedness/`max_align_t`) | Z2 |
| Z-66 | `CerbGlobal.lean` (Z-24), `CerbConcurrency.lean` (Z-25), `CerbDebug.lean`/`CerbUtils.lean` no-op stubs | debug/profiling not on any differential path (effect retirement C1) | **DISPOSED — Z2 `5ed6c4a0a`** (docs/2026-09-04_zero-discrepancy-Z2-record.md §2.8) — was INSTRUMENT; Z2 confirms the debug level is 0 on the oracle in matched mode (`Cerb_debug` default) so `print_debug` output cannot differ | Z2 |
| Z-67 (R1) | `CerbLocation.lean` vs `util/cerb_location.ml` | Z-03 rendering; `isLibraryLocation` (`:180-185`) is NOT correct (the first draft said so wrongly): it matches ANY path segment equal to `libcore`/`include`/`impls`, where the oracle's `is_library_location` (`util/cerb_location.ml:512-520`) tests the RUNTIME-prefixed paths `Cerb_runtime.in_runtime "libc/include"`/`"libcore"`/`"libcore/impls"` — a user file under a directory named `include/` is library-classified on Lean only (D1's substitution then fires wrongly). Added to Z-01's scope: mirror the runtime-prefix check | SEAM INDEX → **FIXED — Z1 `1c1311a57`** (`isLibraryLocation` mirrors the runtime-relative directory set as a suffix test; residual documented in-code: a user file under a directory literally named `runtime/libcore` etc. — the runtime root is not plumbed) | Z1 (done) |
| Z-68 | `CoreParser.lean` vs `parsers/core/core_parser.mly` | Z-01 loc stamping; G6 hash-collision TRIPWIRE (Lean fail-stops where the oracle's digest-keyed symbols cannot collide) | SEAM INDEX → Z-01 FIXED (Z1 `1c1311a57`, `parseLibraryFile`/`stampLibraryFile`); the tripwire is an unreachable-by-construction divergence, declared | Z1 (done) / Z2 |
| Z-69 | `Main.lean` driver glue vs `backend/driver/main.ml` + `pipeline.ml` | Z-24 (flag handling), Z-28 (link order), Z-04 (error text), `--args` mirrored with cites | SEAM INDEX → Z-24 refusal landed (Z1 `b42776130`); Z-28 FIXED (Z3 `2ddc1300c`) | Z1 (done) / Z3 (done) |
| Z-70 | `CabsImport.lean` vs `backend/lean_export/cabs_json.ml` | strict schema, hard errors — the bridge; its failures are `CERB_INCONSISTENT`/bridge classes, never a Lean verdict | **DISPOSED — Z2 `5ed6c4a0a`** (holds; Z2-J-01/J-02 declared in the CabsImport header; docs/2026-09-04_zero-discrepancy-Z2-record.md §2.8) — was INSTRUMENT (bridge); Z2 confirms every parse failure is loud | Z2 |
| Z-71 | instance files (`CerbCabsInstances`, `CerbCtypeInstances`, `CerbFunMapInstances`, `CerbStepInstances`) vs OCaml polymorphic compare / `ctypeEqual` | `CerbCtypeInstances` BEq = annotation-insensitive `ctypeEqual` (the model's own); Ord delegated to the derived compare; `CerbStepInstances` mirrors `driver.lem:1410` blocked-thread equality | **DISPOSED — Z2 `5ed6c4a0a`** (Z2-Q-01 mirrored with `ctype.beq_derived`, Z2-Q-02 settled; docs/2026-09-04_zero-discrepancy-Z2-record.md §2.8) — was Z2-DISPOSED: check each instance against the OCaml equality it replaces (poly-compare vs structural) at every exec use site | Z2 |
...
| **Z3 — libc allocation-order mirror** — DONE 2026-09-05 (`2ddc1300c`; record `docs/2026-09-05_zero-discrepancy-Z3-record.md`) | Z-28 | the 6 pnvi rows (+3 that had joined the class) MATCH in the exhaustive `tests/pnvi_testsuite` spot sweep; `addr_layout.c`-style probe pinned in libc_exec (012) + five immaculate pins; Tier A+B green | S-M |
