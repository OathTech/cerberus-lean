...
/-- The joint stability lemma for the mutual block: above each member's measure,
    any two fuels agree. -/
theorem are_compatible_stable_aux (enumDefs : Fmap sym integerType) (tagDefs : Fmap sym (CerbLocation.Loc × tag_definition)) (k : Nat) :
    (∀ (qs1 : qualifiers) (t1 : ctype) (qs2 : qualifiers) (t2 : ctype) (f g : Nat),
      ...
      are_compatible_lemFuel f enumDefs tagDefs (qs1, t1) (qs2, t2) = are_compatible_lemFuel g enumDefs tagDefs (qs1, t1) (qs2, t2)) ∧
    (∀ (acc : Bool) (l1 l2 : List (qualifiers × ctype × Bool)) (f g : Nat),
      ...
      are_compatible_params_aux_lemFuel f enumDefs tagDefs acc (l1, l2) = are_compatible_params_aux_lemFuel g enumDefs tagDefs acc (l1, l2)) ∧
    (∀ (ps1 ps2 : List (qualifiers × ctype × Bool)) (f g : Nat),
      ...
      are_compatible_params_lemFuel f enumDefs tagDefs ps1 ps2 = are_compatible_params_lemFuel g enumDefs tagDefs ps1 ps2) := by
  induction k with
  | zero =>
      ...
      | succ f =>
        ...
        | succ g =>
          ...
          have key1 : ∀ (q1 : qualifiers) (u1 : ctype) (q2 : qualifiers) (u2 : ctype),
              ...
              are_compatible_lemFuel f enumDefs tagDefs (q1, u1) (q2, u2) = are_compatible_lemFuel g enumDefs tagDefs (q1, u1) (q2, u2) :=
          ...
          have key3 : ∀ (ps1 ps2 : List (qualifiers × ctype × Bool)),
              ...
              are_compatible_params_lemFuel f enumDefs tagDefs ps1 ps2 = are_compatible_params_lemFuel g enumDefs tagDefs ps1 ps2 :=
      ...
      | succ f =>
        ...
        | succ g =>
          ...
          have key2 : ∀ (acc' : Bool) (m1 m2 : List (qualifiers × ctype × Bool)),
              ...
              are_compatible_params_aux_lemFuel f enumDefs tagDefs acc' (m1, m2) = are_compatible_params_aux_lemFuel g enumDefs tagDefs acc' (m1, m2) :=
...
theorem are_compatible_measure_sufficient (enumDefs : Fmap sym integerType) (tagDefs : Fmap sym (CerbLocation.Loc × tag_definition)) (p : qualifiers × ctype) (p0 : qualifiers × ctype) (lemFuel : Nat)
    (lemMeasureLe : ctype.lemSize p.2 + ctype.lemSize p0.2 + 1 ≤ lemFuel) :
    are_compatible_lemFuel lemFuel enumDefs tagDefs p p0 = are_compatible enumDefs tagDefs p p0 := by
  obtain ⟨qs1, t1⟩ := p
  obtain ⟨qs2, t2⟩ := p0
  exact (are_compatible_stable_aux enumDefs tagDefs (ctype.lemSize t1 + ctype.lemSize t2 + 1)).1 qs1 t1 qs2 t2 lemFuel
...
theorem are_compatible_params_aux_measure_sufficient (enumDefs : Fmap sym integerType) (tagDefs : Fmap sym (CerbLocation.Loc × tag_definition)) (acc : Bool)
    ...
    are_compatible_params_aux_lemFuel lemFuel enumDefs tagDefs acc lemTail = are_compatible_params_aux enumDefs tagDefs acc lemTail := by
  obtain ⟨l1, l2⟩ := lemTail
  exact (are_compatible_stable_aux enumDefs tagDefs (ctype_.lemSize_aux1 l1 + ctype_.lemSize_aux1 l2 + 1)).2.1 acc l1 l2 lemFuel
    (ctype_.lemSize_aux1 l1 + ctype_.lemSize_aux1 l2 + 1) (Nat.le_refl _) lemMeasureLe (Nat.le_refl _)

/-- THE OBLIGATION, exactly as AilTypesAux_auxiliary.lean states and delegates it. -/
theorem are_compatible_params_measure_sufficient (enumDefs : Fmap sym integerType) (tagDefs : Fmap sym (CerbLocation.Loc × tag_definition)) (params1 params2 : List (qualifiers × ctype × Bool)) (lemFuel : Nat)
    (lemMeasureLe : ctype_.lemSize_aux1 params1 + ctype_.lemSize_aux1 params2 + 2 ≤ lemFuel) :
    are_compatible_params_lemFuel lemFuel enumDefs tagDefs params1 params2 = are_compatible_params enumDefs tagDefs params1 params2 :=
  (are_compatible_stable_aux enumDefs tagDefs (ctype_.lemSize_aux1 params1 + ctype_.lemSize_aux1 params2 + 2)).2.2 params1 params2 lemFuel
