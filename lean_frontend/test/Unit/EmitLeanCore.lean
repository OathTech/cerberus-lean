...
def ppPtrBase : CerbMem.PointerValueBase → Except String String
  ...
  -- V0 (2026-08-27): the corpus P07/P08 rows' null-pointer literals
  -- (`struct node *hd = 0;`) parse to PVnull — arm added per the
  -- growth contract.
  | .PVnull ty => do
    pure s!"(CerbMem.PointerValueBase.PVnull {← ppCtype ty})"
...
def ppAction_ : generic_action_ Unit sym → Except String String
  ...
  -- V0 (2026-08-27): the corpus P15 row's `i++` (post-increment in a
  -- do-while condition) elaborates to SeqRMW — arm added per the
  -- growth contract (unhandled constructors error loudly, arms are
  -- added deliberately).
...
def ppMemop : generic_memop sym → Except String String
  ...
  -- V0 (2026-08-27): the corpus P07/P08 rows' `while (p)` null tests
  -- elaborate to PtrEq memops — arm added per the growth contract.
  | PtrEq => pure "PtrEq"
...
def slatePlan : List (String × String × String) :=
  -- (2026-08-27 kill-list execution: the t6/t7/e*/c*/x*/z* rows left
  -- with their killed theorems; SlateCore carries the T2–T5 slate.)
  [("t2_add", "add", "addT2"),
...
"/-
  ...
  (the pinned oracle Core dumps of the .c fixtures; the t6/t7/corpus
  terms left this module at the 2026-08-27 kill-list execution with
  their killed concrete-input theorems), plus the
...
def emitSlateModule (texts : List (String × String)) :
  ...
  -- (the batch-4 x3Stdlib params-trio emission was removed at the
  -- 2026-08-27 kill-list execution with the x3 lane)
...
/-! ## The corpus module (V0, 2026-08-27): batch-A program terms.
    The scalar call-boundary rows of the FROZEN target corpus
    (docs/2026-08-27_target-corpus.md) — designated functions (+
    callees, + `struct pt`'s tag definition for P12) emitted from the
    pinned oracle dumps tests/corpus/*.core, plus the ccall-protocol
    params trio from std.core (the x3Stdlib precedent). Same fidelity
    contract as the slate module. -/

/-- fixture-stem → [(function name, def base)] · tagDefs to emit. -/
def corpusPlan :
    List (String × List (String × String) × List (String × String)) :=
  [("p01_clamp", [("clamp0", "clampP01")], []),
   ("p02_sat_add", [("sat_add", "satAddP02")], []),
   ("p03_swap_mayalias",
     [("swap", "swapP03"), ("harness", "harnessP03")], []),
   ("p09_call_contract",
     [("bump", "bumpP09"), ("harness", "harnessP09")], []),
   ("p10_gcd_rec", [("gcd_rec", "gcdRecP10")], []),
   ("p11_gcd_iter", [("gcd", "gcdIterP11")], []),
   ("p12_pt_midpoint",
     [("midpoint", "midpointP12"), ("harness", "harnessP12")],
     [("pt", "structPt")])]

/-- std.core decls the corpus assemblies add beyond the T1 closure
    (the internal-ccall protocol trio; x3Stdlib lineage). -/
def corpusStdPlan : List (String × String) :=
  [("params_length", "paramsLength"),
   ("params_length_aux", "paramsLengthAux"),
   ("params_nth", "paramsNth")]

def corpusHeader : String :=
"/-
  RelSem.CorpusCore — GENERATED by emit-lean-core corpus (V0,
  2026-08-27). DO NOT EDIT.

  Kernel-transparent Lean terms for the FROZEN target corpus's
  batch-A (scalar call-boundary) programs: the parsed ASTs of the
  designated functions (+ callees, + `struct pt`'s tag definition)
  of tests/corpus/{p01,p02,p03,p09,p10,p11,p12}*.core — the pinned
  oracle Core dumps of the frozen lean_frontend/corpus sources —
  plus the ccall-protocol params trio from std.core. Same fidelity
  contract as RelSem/T1Core.lean; `main` deliberately not emitted
  (`callND` never reads it).

  Drift gate: Unit.EmitLeanCoreTest re-parses the pinned inputs,
  re-emits this module, and compares byte-for-byte. Regenerate with:
    .lake/build/bin/emit-lean-core corpus > relsem/RelSem/CorpusCore.lean
-/
...
def corpusFooter : String := "\nend RelSem.Corpus\n"

def emitCorpusModule (texts : List (String × String))
    (stdText : String) : Except String String := do
  let mut out := corpusHeader
  for (stem, fns, tags) in corpusPlan do
    match texts.find? (fun p => p.1 == stem) with
    | none => throw s!"emitCorpusModule: no input text for {stem}"
    | some (_, text) =>
      let cf ← CoreParser.parseFile text
      for (fname, base) in fns do
        out := out ++ (← emitDecl cf fname base) ++ "\n"
      for (tname, tbase) in tags do
        out := out ++ (← emitTagDef cf tname tbase) ++ "\n"
  let std ← CoreParser.parseFile stdText
  for (name, base) in corpusStdPlan do
    out := out ++ (← emitDecl std name base) ++ "\n"
  pure (out ++ corpusFooter)

def readCorpusInputs : IO (List (String × String) × String) := do
  let root ← findRoot
  let fx ← corpusPlan.mapM (fun (stem, _, _) => do
    let text ← IO.FS.readFile (root ++ s!"tests/corpus/{stem}.core")
    pure (stem, text))
  let std ← IO.FS.readFile (root ++ "runtime/libcore/std.core")
  pure (fx, std)

/-! ## The corpus batch-B module (V0): the memory-input rows'
    FUNCTION terms (designated fns + `main` — the whole-program face
    drives main; the SPLICED globals are hand-built parametrically in
    RelSem/CorpusBFiles.lean, so only the fixed code is emitted).
    Inputs: the pinned `tests/corpus/*_funs.core` extractions
    (aggregates + fun-map sections of the oracle dumps; the
    extraction is provenance-checked in test_verify's corpus lane). -/

def corpusBPlan :
    List (String × List (String × String) × List (String × String)) :=
  [("p04_arr_sum", [("arr_sum", "arrSumP04"), ("main", "mainP04")], []),
   ("p05_find_first",
     [("find_first", "findFirstP05"), ("main", "mainP05")], []),
   ("p06_arr_reverse",
     [("arr_reverse", "arrReverseP06"), ("main", "mainP06")], []),
   ("p14_count_pairs",
     [("count_pairs", "countPairsP14"), ("main", "mainP14")], []),
   ("p15_scan_classify",
     [("scan_classify", "scanClassifyP15"), ("main", "mainP15")], []),
   ("p07_list_sum",
     [("list_sum", "listSumP07"), ("main", "mainP07")],
     [("node", "structNodeP07")]),
   ("p08_list_reverse",
     [("list_reverse", "listReverseP08"), ("main", "mainP08")],
     [("node", "structNodeP08")])]

def corpusBHeader : String :=
"/-
  RelSem.CorpusBCore — GENERATED by emit-lean-core corpusb (V0,
  2026-08-27). DO NOT EDIT.

  Kernel-transparent Lean terms for the FROZEN target corpus's
  batch-B (memory-input) programs' FIXED CODE: designated functions
  + `main` (+ `struct node` for P07/P08), parsed from the pinned
  `tests/corpus/*_funs.core` extractions (aggregates + fun-map
  sections of the oracle dumps — the spliced GLOBALS are hand-built
  parametrically in RelSem/CorpusBFiles.lean; only code is emitted
  here). Same fidelity contract as RelSem/CorpusCore.lean.

  Drift gate: Unit.EmitLeanCoreTest re-parses the pinned inputs,
  re-emits this module, and compares byte-for-byte. Regenerate with:
    .lake/build/bin/emit-lean-core corpusb > relsem/RelSem/CorpusBCore.lean
-/
...
def corpusBFooter : String := "\nend RelSem.Corpus\n"

def emitCorpusBModule (texts : List (String × String)) :
    Except String String := do
  let mut out := corpusBHeader
  for (stem, fns, tags) in corpusBPlan do
    match texts.find? (fun p => p.1 == stem) with
    | none => throw s!"emitCorpusBModule: no input text for {stem}"
    | some (_, text) =>
      let cf ← CoreParser.parseFile text
      for (fname, base) in fns do
        out := out ++ (← emitDecl cf fname base) ++ "\n"
      for (tname, tbase) in tags do
        out := out ++ (← emitTagDef cf tname tbase) ++ "\n"
  pure (out ++ corpusBFooter)

def readCorpusBInputs : IO (List (String × String)) := do
  let root ← findRoot
  corpusBPlan.mapM (fun (stem, _, _) => do
    let text ← IO.FS.readFile (root ++ s!"tests/corpus/{stem}_funs.core")
    pure (stem, text))
