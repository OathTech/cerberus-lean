/-
  RelSem.Corpus.C4 — arc-18 R6 batch 2 (2026-08-27): CENSUS-tier c4.

  tests/verify/c4_hexval.c — `int hex_val(int c)`: hex-digit value,
  the three-way range ladder (census rows O3 + §2, uri.c:1559
  is_hex / 1601–1613 decode arms; corpus plan row c4). No locals:
  the READ1 driver-atom shape.
...
def c4Fs : CerbFS.FsState := CerbFS.fs_initial_state

/-- c4's pure spec on driver results: hex_val(102) = 15, Specified. -/
def c4Spec (r : driver_result) : Prop :=
  r.dres_core_value = intValue 15

/-! ## Fixture data (addresses, bytes, memory states — DATA, not
    equations; the T6 scalar-fixture layout family). -/

/-- `hex_val`'s single parameter symbol (the emitted decl's binder). -/
...
def argBytes : List CerbMem.AbsByte := i32 102
...
/-- `hex_val`'s Core body, projected from the emitted (drift-gated)
    declaration. The fallback arm is unreachable (hexvalC4Decl IS a
    `Proc`); a wrong projection would fail every downstream `rfl`
    loudly. -/
def hexvalBody : generic_expr core_run_annotation Unit sym :=
  match fmapLookupBy (fun (s1 s2 : sym) => ordCompare s1 s2) hexvalC4Sym
      c4File.funs with
...
def thRdy : thread_state :=
  { arena := hexvalBody,
    ...
    exec_loc := ELoc_normal [(hexvalC4Sym, CerbLocation.other "RelSem.callND")],
    env := [fmapAddBy (fun (s1 s2 : sym) => ordCompare s1 s2) symX
      (Vobject (OVpointer xPtr)) fmapEmpty],
    current_proc_opt := some hexvalC4Sym }

/-! ## The named-state ladder (minted) -/

/-- Stage 1: driver_globals (e1 has none). -/
derive_state_step dG (seed : Nat)
  from (driver_globals c4File.tagDefs false c4File)
  at (initial_driver_state_threaded seed c4File c4Fs)
...
/-- The ready state (thread set to `hex_val`'s body). -/
...
abbrev rInit (seed : Nat) : driver_state :=
  restOf (initial_driver_state_threaded seed c4File c4Fs)
...
theorem k1_o (seed : Nat) : ∀ bm am,
    app (driver_globals c4File.tagDefs false c4File)
...
@[seg_canon]
theorem c4_canon (seed : Nat) : Seg.CanonAt (rGlob seed) (dG seed) :=
  rfl

@[seg_eq rest]
theorem k3_o (seed : Nat) : ∀ bm am,
    app (resolveFunSym (dG seed).core_file "hex_val")
        (setMaps (rGlob seed) bm am)
      = (NDactive hexvalC4Sym, setMaps (rGlob seed) bm am) :=
  fun _ _ => rfl

@[seg_eq rest]
theorem k4_o (seed : Nat) : ∀ bm am,
    app (lookupFunBody (dG seed).core_file hexvalC4Sym)
        (setMaps (rGlob seed) bm am)
      = (NDactive ([(symX, BTy_object OTy_pointer)], hexvalBody),
         setMaps (rGlob seed) bm am) := fun _ _ => rfl

@[seg_eq rest]
theorem k5_o (seed : Nat) : ∀ bm am,
    app (lookupParamTys (dG seed).core_file hexvalC4Sym)
        (setMaps (rGlob seed) bm am)
...
theorem memValueFromValue_c4_eq :
    memValueFromValue c4File.tagDefs signed_int (intValue 102)
      = some (CerbMem.integerValueMval (Signed Int_)
          (CerbMem.integerIval 102)) := rfl

/-- Stage 6, THE ARGUMENT INJECTION at open maps. -/
@[seg_eq argobj]
theorem k6_o (seed : Nat) : ∀ bm am,
    app (injectArgs c4File.tagDefs 0
          [(symX, BTy_object OTy_pointer)] [signed_int] [intValue 102])
        (setMaps (rGlob seed) bm am)
...
def mkRdy (bm : Std.TreeMap Int CerbMem.AbsByte)
  ...
  { core_file := c4File,
    core_extern := create_extern_symmap c4File,
    core_state0 :=
    ...
    core_run_state0 :=
      { tid_supply := 1, aid_supply := aid, excluded_supply := exc,
        ...
        labeled := (initial_core_run_state_threaded 0
          (collect_labeled_continuations_NEW c4File)).labeled },
...
derive_rounds ro
  ...
  using (c4File.tagDefs) 0 from (mkRdy bm am tr aid exc symc ctr) upto 250 chain builder

/-! ## The final state and the driver atom (READ1: no scratch) -/

/-- The run's terminal value (Specified 15). -/
def v15 : value :=
  Vloaded (LVspecified (OVinteger
    (CerbMem.IntegerValue.IV .Prov_none 15)))

/-- The final driver state at zeroed maps, post `prepare_exit`. -/
noncomputable abbrev roFin0 (seed : Nat) : driver_state :=
  { ro117 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0 with
    core_state0 := prepare_exit
      (ro117 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0).core_state0
      v15 }

/-- The final rest. -/
noncomputable abbrev rDone (seed : Nat) : driver_state := restOf (roFin0 seed)

/-- THE DRIVER LOOP at open maps (read1). -/
@[seg_eq read1]
theorem driver2_o (seed : Nat) : ∀ bm am,
    am.get? 0 = some allocX →
    (∀ i : Nat, (hi : i < argBytes.length) →
      bm.get? (xAddr + (i : Int)) = some (argBytes[i])) →
    app (driver2 c4File.tagDefs false) (setMaps (rRdy seed) bm am)
  ...
  have hchain := ro_chainrel bm am [] 0 0 seed 0 hget hrdX 999881
  have hndct : app (new_drive_core_threads c4File.tagDefs ())
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive [(0, some (Step_done2 v15))],
         ro117 bm am [] 0 0 seed 0) :=
    RelSem.Laws.ndct_offer1 rfl hchain
  have h6 : app (driver2 c4File.tagDefs false)
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive (), setMaps (rDone seed) bm am) := by
    show app (driver2_lemFuel (999999+1) c4File.tagDefs false)
...
/-- c4's FnSpec ([F9], role 1). -/
abbrev hexSpec : Seg.FnSpec Unit :=
  { fname := "hex_val", args := fun _ => [intValue 102],
    post := fun _ => c4Spec }

/-! ## THE THREADED STATEMENTS (fuel-opsem faces, ∀-seed) -/

/-- THE c4 HEADLINE. -/
def C4ThreadedStatement : Prop :=
  ∀ (seed : Nat),
    CallHarnessAdequateThr seed c4File.tagDefs c4File "hex_val"
      [intValue 102] c4Fs c4Spec

/-- **c4 THREADED, UNCONDITIONAL** (trio cone; through the layer). -/
theorem C4Threaded : C4ThreadedStatement := by
  verify_fn hexSpec
  seg_auto

/-- **c4 THREADED UB-freedom** (the safety twin). -/
theorem C4Threaded_ubFree :
    ∀ (seed : Nat),
      CallHarnessUBFreeThr seed c4File.tagDefs c4File "hex_val"
        [intValue 102] c4Fs := by
  verify_fn hexSpec
  seg_auto
