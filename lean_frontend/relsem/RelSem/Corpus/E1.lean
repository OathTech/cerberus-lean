/-
  RelSem.Corpus.E1 — arc-18 R6 batch 1 (2026-08-27): EASY-tier e1.

  tests/verify/e1_clamp.c — `int clamp0(int x)`: single compare +
  select, branch-without-else (census tie: the branch floor; corpus
  plan row e1). No locals: the driver atom is the READ1 walk shape
  (argument object read; maps unchanged; errno rides the frame).
...
def e1Fs : CerbFS.FsState := CerbFS.fs_initial_state

/-- e1's pure spec on driver results: clamp0(-3) = 0, Specified. -/
def e1Spec (r : driver_result) : Prop :=
...
def argBytes : List CerbMem.AbsByte := i32 (-3)
...
/-- `clamp0`'s Core body, projected from the emitted (drift-gated)
    declaration. The fallback arm is unreachable (clampE1Decl IS a
    `Proc`); a wrong projection would fail every downstream `rfl`
    loudly. -/
def clampBody : generic_expr core_run_annotation Unit sym :=
  match fmapLookupBy (fun (s1 s2 : sym) => ordCompare s1 s2) clampE1Sym
      e1File.funs with
...
def thRdy : thread_state :=
  { arena := clampBody,
    ...
    exec_loc := ELoc_normal [(clampE1Sym, CerbLocation.other "RelSem.callND")],
    env := [fmapAddBy (fun (s1 s2 : sym) => ordCompare s1 s2) symX
      (Vobject (OVpointer xPtr)) fmapEmpty],
    current_proc_opt := some clampE1Sym }

/-! ## The named-state ladder (minted) -/

/-- Stage 1: driver_globals (e1 has none). -/
derive_state_step dG (seed : Nat)
  from (driver_globals e1File.tagDefs false e1File)
  at (initial_driver_state_threaded seed e1File e1Fs)
...
/-- The ready state (thread set to `clamp0`'s body). -/
...
abbrev rInit (seed : Nat) : driver_state :=
  restOf (initial_driver_state_threaded seed e1File e1Fs)
...
theorem k1_o (seed : Nat) : ∀ bm am,
    app (driver_globals e1File.tagDefs false e1File)
...
@[seg_canon]
theorem e1_canon (seed : Nat) : Seg.CanonAt (rGlob seed) (dG seed) :=
  rfl

@[seg_eq rest]
theorem k3_o (seed : Nat) : ∀ bm am,
    app (resolveFunSym (dG seed).core_file "clamp0")
        (setMaps (rGlob seed) bm am)
      = (NDactive clampE1Sym, setMaps (rGlob seed) bm am) :=
  fun _ _ => rfl

@[seg_eq rest]
theorem k4_o (seed : Nat) : ∀ bm am,
    app (lookupFunBody (dG seed).core_file clampE1Sym)
        (setMaps (rGlob seed) bm am)
      = (NDactive ([(symX, BTy_object OTy_pointer)], clampBody),
         setMaps (rGlob seed) bm am) := fun _ _ => rfl

@[seg_eq rest]
theorem k5_o (seed : Nat) : ∀ bm am,
    app (lookupParamTys (dG seed).core_file clampE1Sym)
        (setMaps (rGlob seed) bm am)
...
theorem memValueFromValue_e1_eq :
    memValueFromValue e1File.tagDefs signed_int (intValue (-3))
      = some (CerbMem.integerValueMval (Signed Int_)
          (CerbMem.integerIval (-3))) := rfl

/-- Stage 6, THE ARGUMENT INJECTION at open maps. -/
@[seg_eq argobj]
theorem k6_o (seed : Nat) : ∀ bm am,
    app (injectArgs e1File.tagDefs 0
          [(symX, BTy_object OTy_pointer)] [signed_int] [intValue (-3)])
        (setMaps (rGlob seed) bm am)
...
def mkRdy (bm : Std.TreeMap Int CerbMem.AbsByte)
  ...
  { core_file := e1File,
    core_extern := create_extern_symmap e1File,
    core_state0 :=
    ...
    core_run_state0 :=
      { tid_supply := 1, aid_supply := aid, excluded_supply := exc,
        ...
        labeled := (initial_core_run_state_threaded 0
          (collect_labeled_continuations_NEW e1File)).labeled },
...
derive_rounds ro
  ...
  using (e1File.tagDefs) 0 from (mkRdy bm am tr aid exc symc ctr) upto 80 chain builder

/-! ## The final state and the driver atom (READ1: no scratch — the
    run's only memory action is the argument load; maps unchanged) -/
...
noncomputable abbrev roFin0 (seed : Nat) : driver_state :=
  { ro26 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0 with
    core_state0 := prepare_exit
      (ro26 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0).core_state0
      v0 }
...
/-- THE DRIVER LOOP at open maps: characterized by the rest + the
    argument object's footprint; NO memory writes (read1 — the errno
    object rides the frame, maps come back unchanged). -/
@[seg_eq read1]
theorem driver2_o (seed : Nat) : ∀ bm am,
    am.get? 0 = some allocX →
    (∀ i : Nat, (hi : i < argBytes.length) →
      bm.get? (xAddr + (i : Int)) = some (argBytes[i])) →
    app (driver2 e1File.tagDefs false) (setMaps (rRdy seed) bm am)
  ...
  have hchain := ro_chainrel bm am [] 0 0 seed 0 hget hrdX 999972
  have hndct : app (new_drive_core_threads e1File.tagDefs ())
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive [(0, some (Step_done2 v0))],
         ro26 bm am [] 0 0 seed 0) :=
    RelSem.Laws.ndct_offer1 rfl hchain
  have h6 : app (driver2 e1File.tagDefs false)
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive (), setMaps (rDone seed) bm am) := by
    show app (driver2_lemFuel (999999+1) e1File.tagDefs false)
...
/-- e1's FnSpec ([F9], role 1): `clamp0(-3)` returns `Specified 0`.
    REDUCIBLE (abbrev) — the faces unify its projections against the
    statement text. -/
abbrev clampSpec : Seg.FnSpec Unit :=
  { fname := "clamp0", args := fun _ => [intValue (-3)],
    post := fun _ => e1Spec }

/-! ## THE THREADED STATEMENTS (fuel-opsem faces, ∀-seed) -/

/-- THE e1 HEADLINE (fuel opsem only): for EVERY fresh-symbol supply
    seed, every outcome the production runner enumerates for
    `callND(clamp0, [intValue (-3)])` from the threaded initial state
    is `Active r` with `r.dres_core_value = intValue 0`. -/
def E1ThreadedStatement : Prop :=
  ∀ (seed : Nat),
    CallHarnessAdequateThr seed e1File.tagDefs e1File "clamp0"
      [intValue (-3)] e1Fs e1Spec

/-- **e1 THREADED, UNCONDITIONAL** (cone exactly the classical trio;
    through the segment layer). -/
theorem E1Threaded : E1ThreadedStatement := by
  verify_fn clampSpec
  seg_auto

/-- **e1 THREADED UB-freedom** (the safety twin — same route). -/
theorem E1Threaded_ubFree :
    ∀ (seed : Nat),
      CallHarnessUBFreeThr seed e1File.tagDefs e1File "clamp0"
        [intValue (-3)] e1Fs := by
  verify_fn clampSpec
  seg_auto
