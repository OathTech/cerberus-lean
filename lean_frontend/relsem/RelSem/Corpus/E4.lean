/-
  RelSem.Corpus.E4 — arc-18 R6 batch 1 (2026-08-27): EASY-tier e4.

  tests/verify/e4_isdigit.c — `int is_digit(int c)`: range-pair
  char-class predicate, short-circuit && (census row §2, the
  uri.c:127 ISA_DIGIT shape; corpus plan row e4). No locals: the
  READ1 driver-atom shape.
...
def e4Fs : CerbFS.FsState := CerbFS.fs_initial_state

/-- e4's pure spec on driver results: is_digit(53) = 1, Specified. -/
def e4Spec (r : driver_result) : Prop :=
  r.dres_core_value = intValue 1

/-! ## Fixture data (addresses, bytes, memory states — DATA, not
    equations; the T6 scalar-fixture layout family). -/

/-- the single parameter symbol (the emitted decl's binder). -/
def symX : sym := Symbol "" 5097672513440128799 (SD_Id "c")
...
/-- The argument object's byte image (arg = 53). -/
def argBytes : List CerbMem.AbsByte := i32 (53)
...
/-- `is_digit`'s Core body, projected from the emitted (drift-gated)
    declaration. The fallback arm is unreachable (isdigitE4Decl IS a
    `Proc`); a wrong projection would fail every downstream `rfl`
    loudly. -/
def is_digitBody : generic_expr core_run_annotation Unit sym :=
  match fmapLookupBy (fun (s1 s2 : sym) => ordCompare s1 s2) isdigitE4Sym
      e4File.funs with
...
def thRdy : thread_state :=
  { arena := is_digitBody,
    ...
    exec_loc := ELoc_normal [(isdigitE4Sym, CerbLocation.other "RelSem.callND")],
    env := [fmapAddBy (fun (s1 s2 : sym) => ordCompare s1 s2) symX
      (Vobject (OVpointer xPtr)) fmapEmpty],
    current_proc_opt := some isdigitE4Sym }

/-! ## The named-state ladder (minted) -/

/-- Stage 1: driver_globals (e1 has none). -/
derive_state_step dG (seed : Nat)
  from (driver_globals e4File.tagDefs false e4File)
  at (initial_driver_state_threaded seed e4File e4Fs)
...
/-- The ready state (thread set to `is_digit`'s body). -/
...
abbrev rInit (seed : Nat) : driver_state :=
  restOf (initial_driver_state_threaded seed e4File e4Fs)
...
theorem k1_o (seed : Nat) : ∀ bm am,
    app (driver_globals e4File.tagDefs false e4File)
...
@[seg_canon]
theorem e4_canon (seed : Nat) : Seg.CanonAt (rGlob seed) (dG seed) :=
  rfl

@[seg_eq rest]
theorem k3_o (seed : Nat) : ∀ bm am,
    app (resolveFunSym (dG seed).core_file "is_digit")
        (setMaps (rGlob seed) bm am)
      = (NDactive isdigitE4Sym, setMaps (rGlob seed) bm am) :=
  fun _ _ => rfl

@[seg_eq rest]
theorem k4_o (seed : Nat) : ∀ bm am,
    app (lookupFunBody (dG seed).core_file isdigitE4Sym)
        (setMaps (rGlob seed) bm am)
      = (NDactive ([(symX, BTy_object OTy_pointer)], is_digitBody),
         setMaps (rGlob seed) bm am) := fun _ _ => rfl

@[seg_eq rest]
theorem k5_o (seed : Nat) : ∀ bm am,
    app (lookupParamTys (dG seed).core_file isdigitE4Sym)
        (setMaps (rGlob seed) bm am)
...
theorem memValueFromValue_e4_eq :
    memValueFromValue e4File.tagDefs signed_int (intValue (53))
      = some (CerbMem.integerValueMval (Signed Int_)
          (CerbMem.integerIval (53))) := rfl

/-- Stage 6, THE ARGUMENT INJECTION at open maps. -/
@[seg_eq argobj]
theorem k6_o (seed : Nat) : ∀ bm am,
    app (injectArgs e4File.tagDefs 0
          [(symX, BTy_object OTy_pointer)] [signed_int] [intValue (53)])
        (setMaps (rGlob seed) bm am)
...
def mkRdy (bm : Std.TreeMap Int CerbMem.AbsByte)
  ...
  { core_file := e4File,
    core_extern := create_extern_symmap e4File,
    core_state0 :=
    ...
    core_run_state0 :=
      { tid_supply := 1, aid_supply := aid, excluded_supply := exc,
        ...
        labeled := (initial_core_run_state_threaded 0
          (collect_labeled_continuations_NEW e4File)).labeled },
...
derive_rounds ro
  ...
  using (e4File.tagDefs) 0 from (mkRdy bm am tr aid exc symc ctr) upto 120 chain builder

/-! ## The final state and the driver atom (READ1: no scratch — maps
    unchanged; the errno object rides the frame) -/

/-- The run's terminal value (Specified 1). -/
def v1 : value :=
  Vloaded (LVspecified (OVinteger
    (CerbMem.IntegerValue.IV .Prov_none 1)))
...
noncomputable abbrev roFin0 (seed : Nat) : driver_state :=
  { ro55 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0 with
    core_state0 := prepare_exit
      (ro55 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0).core_state0
      v1 }
...
theorem driver2_o (seed : Nat) : ∀ bm am,
    ...
    app (driver2 e4File.tagDefs false) (setMaps (rRdy seed) bm am)
  ...
  have hchain := ro_chainrel bm am [] 0 0 seed 0 hget hrdX 999943
  have hndct : app (new_drive_core_threads e4File.tagDefs ())
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive [(0, some (Step_done2 v1))],
         ro55 bm am [] 0 0 seed 0) :=
    RelSem.Laws.ndct_offer1 rfl hchain
  have h6 : app (driver2 e4File.tagDefs false)
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive (), setMaps (rDone seed) bm am) := by
    show app (driver2_lemFuel (999999+1) e4File.tagDefs false)
...
/-- e4's FnSpec ([F9], role 1). REDUCIBLE — the faces unify its
    projections against the statement text. -/
abbrev digitSpec : Seg.FnSpec Unit :=
  { fname := "is_digit", args := fun _ => [intValue 53],
    post := fun _ => e4Spec }

/-! ## THE THREADED STATEMENTS (fuel-opsem faces, ∀-seed) -/

/-- THE e4 HEADLINE (fuel opsem only): for EVERY fresh-symbol supply
    seed, every outcome of `callND(is_digit, [intValue 53])` from the
    threaded initial state is `Active r` meeting the spec. -/
def E4ThreadedStatement : Prop :=
  ∀ (seed : Nat),
    CallHarnessAdequateThr seed e4File.tagDefs e4File "is_digit"
      [intValue 53] e4Fs e4Spec

/-- **e4 THREADED, UNCONDITIONAL** (cone exactly the classical trio;
    through the segment layer). -/
theorem E4Threaded : E4ThreadedStatement := by
  verify_fn digitSpec
  seg_auto

/-- **e4 THREADED UB-freedom** (the safety twin — same route). -/
theorem E4Threaded_ubFree :
    ∀ (seed : Nat),
      CallHarnessUBFreeThr seed e4File.tagDefs e4File "is_digit"
        [intValue 53] e4Fs := by
  verify_fn digitSpec
  seg_auto
