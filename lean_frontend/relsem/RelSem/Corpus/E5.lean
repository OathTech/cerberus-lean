/-
  RelSem.Corpus.E5 — arc-18 R6 batch 1 (2026-08-27): EASY-tier e5.

  tests/verify/e5_ismark.c — `int is_mark(int c)`: 9-arm literal
  disjunction chain, short-circuit || ladder (census row §2, the
  uri.c:79 IS_MARK shape; corpus plan row e5). No locals: the READ1
  driver-atom shape.
...
def e5Fs : CerbFS.FsState := CerbFS.fs_initial_state

/-- e5's pure spec on driver results: is_mark(42) = 1, Specified. -/
def e5Spec (r : driver_result) : Prop :=
...
/-- The argument object's byte image (arg = 42). -/
def argBytes : List CerbMem.AbsByte := i32 (42)
...
/-- `is_mark`'s Core body, projected from the emitted (drift-gated)
    declaration. The fallback arm is unreachable (ismarkE5Decl IS a
    `Proc`); a wrong projection would fail every downstream `rfl`
    loudly. -/
def is_markBody : generic_expr core_run_annotation Unit sym :=
  match fmapLookupBy (fun (s1 s2 : sym) => ordCompare s1 s2) ismarkE5Sym
      e5File.funs with
...
def thRdy : thread_state :=
  { arena := is_markBody,
    ...
    exec_loc := ELoc_normal [(ismarkE5Sym, CerbLocation.other "RelSem.callND")],
    env := [fmapAddBy (fun (s1 s2 : sym) => ordCompare s1 s2) symX
      (Vobject (OVpointer xPtr)) fmapEmpty],
    current_proc_opt := some ismarkE5Sym }

/-! ## The named-state ladder (minted) -/

/-- Stage 1: driver_globals (e1 has none). -/
derive_state_step dG (seed : Nat)
  from (driver_globals e5File.tagDefs false e5File)
  at (initial_driver_state_threaded seed e5File e5Fs)
...
/-- The ready state (thread set to `is_mark`'s body). -/
...
abbrev rInit (seed : Nat) : driver_state :=
  restOf (initial_driver_state_threaded seed e5File e5Fs)
...
theorem k1_o (seed : Nat) : ∀ bm am,
    app (driver_globals e5File.tagDefs false e5File)
...
@[seg_canon]
theorem e5_canon (seed : Nat) : Seg.CanonAt (rGlob seed) (dG seed) :=
  rfl

@[seg_eq rest]
theorem k3_o (seed : Nat) : ∀ bm am,
    app (resolveFunSym (dG seed).core_file "is_mark")
        (setMaps (rGlob seed) bm am)
      = (NDactive ismarkE5Sym, setMaps (rGlob seed) bm am) :=
  fun _ _ => rfl

@[seg_eq rest]
theorem k4_o (seed : Nat) : ∀ bm am,
    app (lookupFunBody (dG seed).core_file ismarkE5Sym)
        (setMaps (rGlob seed) bm am)
      = (NDactive ([(symX, BTy_object OTy_pointer)], is_markBody),
         setMaps (rGlob seed) bm am) := fun _ _ => rfl

@[seg_eq rest]
theorem k5_o (seed : Nat) : ∀ bm am,
    app (lookupParamTys (dG seed).core_file ismarkE5Sym)
        (setMaps (rGlob seed) bm am)
...
theorem memValueFromValue_e5_eq :
    memValueFromValue e5File.tagDefs signed_int (intValue (42))
      = some (CerbMem.integerValueMval (Signed Int_)
          (CerbMem.integerIval (42))) := rfl

/-- Stage 6, THE ARGUMENT INJECTION at open maps. -/
@[seg_eq argobj]
theorem k6_o (seed : Nat) : ∀ bm am,
    app (injectArgs e5File.tagDefs 0
          [(symX, BTy_object OTy_pointer)] [signed_int] [intValue (42)])
        (setMaps (rGlob seed) bm am)
...
def mkRdy (bm : Std.TreeMap Int CerbMem.AbsByte)
  ...
  { core_file := e5File,
    core_extern := create_extern_symmap e5File,
    core_state0 :=
    ...
    core_run_state0 :=
      { tid_supply := 1, aid_supply := aid, excluded_supply := exc,
        ...
        labeled := (initial_core_run_state_threaded 0
          (collect_labeled_continuations_NEW e5File)).labeled },
...
derive_rounds ro
  ...
  using (e5File.tagDefs) 0 from (mkRdy bm am tr aid exc symc ctr) upto 400 chain builder
...
noncomputable abbrev roFin0 (seed : Nat) : driver_state :=
  { ro204 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0 with
    core_state0 := prepare_exit
      (ro204 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0).core_state0
      v1 }
...
theorem driver2_o (seed : Nat) : ∀ bm am,
    ...
    app (driver2 e5File.tagDefs false) (setMaps (rRdy seed) bm am)
  ...
  have hchain := ro_chainrel bm am [] 0 0 seed 0 hget hrdX 999794
  have hndct : app (new_drive_core_threads e5File.tagDefs ())
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive [(0, some (Step_done2 v1))],
         ro204 bm am [] 0 0 seed 0) :=
    RelSem.Laws.ndct_offer1 rfl hchain
  have h6 : app (driver2 e5File.tagDefs false)
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive (), setMaps (rDone seed) bm am) := by
    show app (driver2_lemFuel (999999+1) e5File.tagDefs false)
...
/-- e5's FnSpec ([F9], role 1). REDUCIBLE — the faces unify its
    projections against the statement text. -/
abbrev markSpec : Seg.FnSpec Unit :=
  { fname := "is_mark", args := fun _ => [intValue 42],
    post := fun _ => e5Spec }

/-! ## THE THREADED STATEMENTS (fuel-opsem faces, ∀-seed) -/

/-- THE e5 HEADLINE (fuel opsem only): for EVERY fresh-symbol supply
    seed, every outcome of `callND(is_mark, [intValue 42])` from the
    threaded initial state is `Active r` meeting the spec. -/
def E5ThreadedStatement : Prop :=
  ∀ (seed : Nat),
    CallHarnessAdequateThr seed e5File.tagDefs e5File "is_mark"
      [intValue 42] e5Fs e5Spec

/-- **e5 THREADED, UNCONDITIONAL** (cone exactly the classical trio;
    through the segment layer). -/
theorem E5Threaded : E5ThreadedStatement := by
  verify_fn markSpec
  seg_auto

/-- **e5 THREADED UB-freedom** (the safety twin — same route). -/
theorem E5Threaded_ubFree :
    ∀ (seed : Nat),
      CallHarnessUBFreeThr seed e5File.tagDefs e5File "is_mark"
        [intValue 42] e5Fs := by
  verify_fn markSpec
  seg_auto
