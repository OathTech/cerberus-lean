/-
  RelSem.Corpus.C5 — arc-18 R6 batch 2 (2026-08-27): CENSUS-tier c5.

  tests/verify/c5_pcthi.c — `int pct_hi(int v)`: percent-encode
  high-nibble arithmetic (census row O2, uri.c:1140–1142; corpus
  plan row c5). One local: the SCRATCH1 driver-atom shape.
...
def c5Fs : CerbFS.FsState := CerbFS.fs_initial_state

/-- c5's pure spec on driver results: pct_hi(65) = 52, Specified. -/
def c5Spec (r : driver_result) : Prop :=
  r.dres_core_value = intValue 52

/-! ## Fixture data (addresses, bytes, memory states — DATA, not
    equations; the T6 scalar-fixture layout family). -/

/-- `pct_hi`'s single parameter symbol (the emitted decl's binder). -/
def symX : sym := Symbol "" 1965435164061188486 (SD_Id "v")
...
def argBytes : List CerbMem.AbsByte := i32 65
...
/-- `pct_hi`'s Core body, projected from the emitted (drift-gated)
    declaration. The fallback arm is unreachable (pcthiC5Decl IS a
    `Proc`); a wrong projection would fail every downstream `rfl`
    loudly. -/
def pcthiBody : generic_expr core_run_annotation Unit sym :=
  match fmapLookupBy (fun (s1 s2 : sym) => ordCompare s1 s2) pcthiC5Sym
      c5File.funs with
...
def thRdy : thread_state :=
  { arena := pcthiBody,
    ...
    exec_loc := ELoc_normal [(pcthiC5Sym, CerbLocation.other "RelSem.callND")],
    env := [fmapAddBy (fun (s1 s2 : sym) => ordCompare s1 s2) symX
      (Vobject (OVpointer xPtr)) fmapEmpty],
    current_proc_opt := some pcthiC5Sym }

/-! ## The named-state ladder (minted) -/

/-- Stage 1: driver_globals (e1 has none). -/
derive_state_step dG (seed : Nat)
  from (driver_globals c5File.tagDefs false c5File)
  at (initial_driver_state_threaded seed c5File c5Fs)
...
/-- The ready state (thread set to `pct_hi`'s body). -/
...
abbrev rInit (seed : Nat) : driver_state :=
  restOf (initial_driver_state_threaded seed c5File c5Fs)
...
theorem k1_o (seed : Nat) : ∀ bm am,
    app (driver_globals c5File.tagDefs false c5File)
...
@[seg_canon]
theorem c5_canon (seed : Nat) : Seg.CanonAt (rGlob seed) (dG seed) :=
  rfl

@[seg_eq rest]
theorem k3_o (seed : Nat) : ∀ bm am,
    app (resolveFunSym (dG seed).core_file "pct_hi")
        (setMaps (rGlob seed) bm am)
      = (NDactive pcthiC5Sym, setMaps (rGlob seed) bm am) :=
  fun _ _ => rfl

@[seg_eq rest]
theorem k4_o (seed : Nat) : ∀ bm am,
    app (lookupFunBody (dG seed).core_file pcthiC5Sym)
        (setMaps (rGlob seed) bm am)
      = (NDactive ([(symX, BTy_object OTy_pointer)], pcthiBody),
         setMaps (rGlob seed) bm am) := fun _ _ => rfl

@[seg_eq rest]
theorem k5_o (seed : Nat) : ∀ bm am,
    app (lookupParamTys (dG seed).core_file pcthiC5Sym)
        (setMaps (rGlob seed) bm am)
...
theorem memValueFromValue_c5_eq :
    memValueFromValue c5File.tagDefs signed_int (intValue 65)
      = some (CerbMem.integerValueMval (Signed Int_)
          (CerbMem.integerIval 65)) := rfl

/-- Stage 6, THE ARGUMENT INJECTION at open maps. -/
@[seg_eq argobj]
theorem k6_o (seed : Nat) : ∀ bm am,
    app (injectArgs c5File.tagDefs 0
          [(symX, BTy_object OTy_pointer)] [signed_int] [intValue 65])
        (setMaps (rGlob seed) bm am)
...
def mkRdy (bm : Std.TreeMap Int CerbMem.AbsByte)
  ...
  { core_file := c5File,
    core_extern := create_extern_symmap c5File,
    core_state0 :=
    ...
    core_run_state0 :=
      { tid_supply := 1, aid_supply := aid, excluded_supply := exc,
        ...
        labeled := (initial_core_run_state_threaded 0
          (collect_labeled_continuations_NEW c5File)).labeled },
...
derive_rounds ro
  ...
  using (c5File.tagDefs) 0 from (mkRdy bm am tr aid exc symc ctr) upto 120 chain builder

/-! ## The scratch object (hi, pct_hi's local) and the final state -/

/-- hi's address (the bump allocator's next slot below errno). -/
def tAddr : Int := 281474976710640

/-- hi's allocation record (rule-shaped). -/
def allocY : CerbMem.Allocation :=
  { base := tAddr, size := 4, ty := some signed_int,
    prefix_ := PrefOther "Core" }

/-- hi's stored byte image (hi = 65 / 16 = 4). -/
def bytesY : List CerbMem.AbsByte := i32 4

/-- The run's terminal value (Specified 52). -/
def v52 : value :=
  Vloaded (LVspecified (OVinteger
    (CerbMem.IntegerValue.IV .Prov_none 52)))

/-- The scratch address arithmetic. -/
@[seg_fact]
...
noncomputable abbrev roFin0 (seed : Nat) : driver_state :=
  { ro53 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0 with
    core_state0 := prepare_exit
      (ro53 Std.TreeMap.empty Std.TreeMap.empty [] 0 0 seed 0).core_state0
      v52 }

/-- The final rest. -/
noncomputable abbrev rDone (seed : Nat) : driver_state := restOf (roFin0 seed)

/-- THE DRIVER LOOP at open maps (scratch1: hi's whole lifetime
    internal; the errno object rides the frame). -/
@[seg_eq scratch1]
theorem driver2_o (seed : Nat) : ∀ bm am,
    am.get? 0 = some allocX →
    (∀ i : Nat, (hi : i < argBytes.length) →
      bm.get? (xAddr + (i : Int)) = some (argBytes[i])) →
    app (driver2 c5File.tagDefs false) (setMaps (rRdy seed) bm am)
  ...
  have hchain := ro_chainrel bm am [] 0 0 seed 0 hget hrdX 999945
  have hndct : app (new_drive_core_threads c5File.tagDefs ())
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive [(0, some (Step_done2 v52))],
         ro53 bm am [] 0 0 seed 0) :=
    RelSem.Laws.ndct_offer1 rfl hchain
  have h6 : app (driver2 c5File.tagDefs false)
      (mkRdy bm am [] 0 0 seed 0)
      = (NDactive (), setMaps (rDone seed)
          (allocStoreBytes bm tAddr 4 bytesY)
          ((am.insert 2 allocY).erase 2)) := by
    show app (driver2_lemFuel (999999+1) c5File.tagDefs false)
...
/-- c5's FnSpec ([F9], role 1). -/
abbrev pctSpec : Seg.FnSpec Unit :=
  { fname := "pct_hi", args := fun _ => [intValue 65],
    post := fun _ => c5Spec }

/-! ## THE THREADED STATEMENTS (fuel-opsem faces, ∀-seed) -/

/-- THE c5 HEADLINE. -/
def C5ThreadedStatement : Prop :=
  ∀ (seed : Nat),
    CallHarnessAdequateThr seed c5File.tagDefs c5File "pct_hi"
      [intValue 65] c5Fs c5Spec

/-- **c5 THREADED, UNCONDITIONAL** (trio cone; through the layer). -/
theorem C5Threaded : C5ThreadedStatement := by
  verify_fn pctSpec
  seg_auto

/-- **c5 THREADED UB-freedom** (the safety twin). -/
theorem C5Threaded_ubFree :
    ∀ (seed : Nat),
      CallHarnessUBFreeThr seed c5File.tagDefs c5File "pct_hi"
        [intValue 65] c5Fs := by
  verify_fn pctSpec
  seg_auto
