...
/-- P01 baseline: the identical statement over the SUPPLY walk
    (the A/B cost control). -/
theorem probe_p01_full_supply (x : Int) (seed : Nat) [CerbStGS CerbStS]
    (hx1 : -2147483648 ≤ x) (hx2 : x ≤ 2147483647) :
    (Ctx.interp (GF := CerbStS)
      ⟨RelSem.P01.p01Ctl0, ⟨1, 0, 0, seed⟩, env0, mr2, al0, bs0 x⟩) ⊢
      WP (dnmsK RelSem.Corpus.p01File.tagDefs 1000000 fmapEmpty 0 []
...
-- Cone check (probe): the mint path must add NOTHING beyond the
-- classical trio.
