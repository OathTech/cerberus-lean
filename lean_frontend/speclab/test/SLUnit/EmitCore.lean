...
def ppImplConst : implementation_constant → Except String String
  ...
  -- arc-15 S5: std.core `ctype_width` body (`<bits_in_byte>`) — the
  -- rung's one emitter delta (an impl-constant table row, not a new
  -- pp surface)
  | Characters__bits_in_byte => pure "Characters__bits_in_byte"
...
/-! ## The tree-rung S4 emission plan (R4: rotate_right — THE
REFERENCE INSTANCE).

The rotate parametric main: instances a/b/d are the pinned 6-node
shape (path [l]) with pre-order element wire bytes 1..24 / 101..124 /
201..224 — all 24 byte positions' value TRIPLES pairwise distinct;
each byte appears at TWO sites (choices[] + its rotation-permuted
position in expected[]), so 24 parameters cover 48 substitution
sites. The out-of-trio pin is c = vals [0, -1, INT_MIN, INT_MAX, 1,
-INT_MAX] (boundary bit patterns). The R4 TU carries FIVE procs
beyond the target family firsts: the recursive helper trio + free
(scan_tree/build_tree/serialize_tree/free_tree) — the S3
symbol-numbering-coupling finding now bites PER HELPER: the
verbatim-path instances (root, deep) and the build-only instance have
structurally different mains, so their ENTIRE TU decl sets are pinned
from their own dumps; the two plant TUs share the healthy main
structure and their helper decls are ASSERTED byte-identical to the
a-dump's (loud error otherwise), so only their targets + mains are
pinned separately.

No new pp surfaces at R4 (measured against the S3 emitter: same
memop/action/UB families) — the fidelity contract's loud errors are
the check. -/

/-- (valueA, valueB, valueD) → parameter name, rotate trio: wire byte
i+1 ↦ `bi`. -/
def treeParamTable : List ((String × String × String) × String) :=
  (List.range 24).map fun i =>
    ((toString (i + 1), toString (i + 101), toString (i + 201)),
      s!"b{i}")

def treeModuleHeader : String :=
"/-
  SpecLab.TreeRotCore — GENERATED by speclab-emit-tree (arc-15 S4).
  DO NOT EDIT.

  Kernel-transparent Lean terms for the R4 tree-rotation harness
  family (THE REFERENCE INSTANCE): rotate_right (fresh authorship —
  see SpecLab/TreeRot.lean) + the `struct node` TAG DEFINITION + the
  recursive helper trio (scan_tree/build_tree/serialize_tree/
  free_tree) + the rotate PARAMETRIC main (a function of the
  twenty-four pre-order element wire bytes b0..b23 — 48 substitution
  sites, expected[] derived; digit-run zip over the pinned instances
  a/b/d, wire bytes 1..24 / 101..124 / 201..224 —
  tests/speclab/rotate_{a,b,d}.core), the VERBATIM path instances
  (rotate_{root,deep}.core — the pointer-selection statement family's
  non-parametric paths, each TU pinned whole per the
  symbol-numbering-coupling finding), the WRONG-CHILD-SWAP and
  DROPPED-SUBTREE PLANT decls (rotate_{swapplant,dropplant}.core —
  helper decls asserted byte-identical to the a-dump's and shared),
  and the BUILD-ONLY TU (rotate_build.core, pinned whole) — exactly
  as CoreParser produces them (fidelity contract of SLUnit.EmitCore).
  The std.core closure is SHARED with SpecLab.DivModCore +
  SpecLab.ListAppendCore (drift-gated there).

  Drift gate: SLUnit.TreeGateTest re-parses the pinned inputs,
  re-emits this module byte-for-byte, pins the parametric main back
  to ALL FOUR pinned healthy dumps (incl. the out-of-trio c), runs
  the assembled files through the generated `drive` at the pinned
  verdicts, and checks THE LEAK OBSERVABLE (final allocation map) on
  every run. Regenerate:
    .lake/build/bin/speclab-emit-tree > SpecLab/TreeRotCore.lean
-/

import Core

set_option autoImplicit false
-- parametric-main helpers bind (b0 .. b23) uniformly; helpers
-- without literal sites do not reference them (generated file,
-- cosmetic)
set_option linter.unusedVariables false

namespace SpecLab.TreeRotCore

"

def treeModuleFooter : String := "\nend SpecLab.TreeRotCore\n"

/-- Emit the struct node tag definition (sym + tag_definition). -/
def emitNodeTagDef (cf : CoreFile) : Except String String := do
  match cf.tagDefs with
  | [(s, (loc, td))] =>
    let _ ← ppLoc loc  -- errors on non-unknown
    pure ("/-- `struct node`: tag symbol, as interned by CoreParser. -/\n" ++
      s!"def nodeSym : sym :=\n  {← ppSym s}\n\n" ++
      "/-- `struct node`: the tag definition, verbatim. -/\n" ++
      s!"def nodeTagDef : tag_definition :=\n  {← ppTagDef td}\n")
  | _ => throw s!"emitNodeTagDef: expected exactly one tag def, got {cf.tagDefs.length}"

/-- The tree TU's non-main decl plan (target + the helper trio +
free). -/
def treeDeclPlan : List (String × String) :=
  [("rotate_right", "rotateRight"), ("scan_tree", "scanTree"),
   ("build_tree", "buildTree"), ("serialize_tree", "serializeTree"),
   ("free_tree", "freeTree")]

/-- Emit one whole tree TU (non-main decls under `prefix_` names +
its main, verbatim). -/
def emitTreeTU (cf : CoreFile) (pfx : String) : Except String String := do
  let mut out := ""
  for (name, base) in treeDeclPlan do
    out := out ++ (← emitDeclH cf name (pfx ++ base.capitalize)) ++ "\n"
  -- emitDeclH provides both {pfx}MainSym and {pfx}MainDecl
  out := out ++ (← emitDeclH cf "main" (pfx ++ "Main")) ++ "\n"
  pure out

def emitTreeModule (da db dd dc droot ddeep dsp ddp dbuild : String) :
    Except String String := do
  let cfA ← CoreParser.parseFile da
  let cfB ← CoreParser.parseFile db
  let cfD ← CoreParser.parseFile dd
  let cfC ← CoreParser.parseFile dc
  let cfRoot ← CoreParser.parseFile droot
  let cfDeep ← CoreParser.parseFile ddeep
  let cfSP ← CoreParser.parseFile dsp
  let cfDP ← CoreParser.parseFile ddp
  let cfBu ← CoreParser.parseFile dbuild
  -- the tag def must be identical across all nine dumps
  let tagA ← emitNodeTagDef cfA
  for (lbl, cf) in [("b", cfB), ("d", cfD), ("c", cfC), ("root", cfRoot),
      ("deep", cfDeep), ("sp", cfSP), ("dp", cfDP), ("build", cfBu)] do
    if (← emitNodeTagDef cf) ≠ tagA then
      throw s!"emitTreeModule: tag def differs in dump {lbl}"
  -- target + helpers must be literally identical across the four
  -- parametric-family dumps (identical main STRUCTURE ⇒ identical
  -- fresh numbering)
  for (name, _) in treeDeclPlan do
    let t ← declText cfA name
    for (lbl, cf) in [("b", cfB), ("d", cfD), ("c", cfC)] do
      if (← declText cf name) ≠ t then
        throw s!"emitTreeModule: '{name}' differs in dump {lbl}"
  -- the plant TUs share the healthy main structure: their HELPER
  -- decls must be byte-identical to the a-dump's (measured at S4;
  -- loud error = the sharing assumption broke)
  for (name, _) in treeDeclPlan do
    if name == "rotate_right" then continue
    let t ← declText cfA name
    for (lbl, cf) in [("swap plant", cfSP), ("drop plant", cfDP)] do
      if (← declText cf name) ≠ t then
        throw s!"emitTreeModule: helper '{name}' differs in the {lbl} dump — the plant TUs no longer share the healthy numbering"
  -- the rotate parametric main: per-instance FULL blocks zipped
  -- whole (hoisting decisions literal-independent — the S2
  -- mechanism)
  let (mainSymA, _) ← findDecl cfA "main"
  let sig := " (b0 b1 b2 b3 b4 b5 b6 b7 b8 b9 b10 b11 b12 b13 b14 b15 b16 b17 b18 b19 b20 b21 b22 b23 : Int)"
  let app := " b0 b1 b2 b3 b4 b5 b6 b7 b8 b9 b10 b11 b12 b13 b14 b15 b16 b17 b18 b19 b20 b21 b22 b23"
  let ba ← emitMainBlockP cfA "rotateMainParamDecl" sig app
  let bb ← emitMainBlockP cfB "rotateMainParamDecl" sig app
  let bd ← emitMainBlockP cfD "rotateMainParamDecl" sig app
  let pblock ← zipParamWith treeParamTable 48 ba bb bd
  let mut out := treeModuleHeader
  out := out ++ tagA ++ "\n"
  for (name, base) in treeDeclPlan do
    out := out ++ (← emitDeclH cfA name base) ++ "\n"
  out := out ++ s!"/-- `main` (parametric family): symbol, as interned by CoreParser. -/\n"
  out := out ++ s!"def mainSym : sym :=\n  {← ppSym mainSymA}\n\n"
  out := out ++ "/- rotate `main`, PARAMETRIC in the twenty-four pre-order element\nwire bytes (see the module header; expected[] sites are DERIVED —\nthey share the parameters; hoisted helpers below carry the binders).\n`rotateMainParamDecl 1 2 ... 24` is the parsed main of the pinned\ninstance a, byte-for-byte (SLUnit.TreeGateTest). -/\n"
  out := out ++ pblock ++ "\n"
  out := out ++ (← emitTreeTU cfRoot "root")
  out := out ++ (← emitTreeTU cfDeep "deep")
  out := out ++ (← emitDeclH cfSP "rotate_right" "swapRotateRight") ++ "\n"
  out := out ++ (← emitDeclH cfSP "main" "swapMain") ++ "\n"
  out := out ++ (← emitDeclH cfDP "rotate_right" "dropRotateRight") ++ "\n"
  out := out ++ (← emitDeclH cfDP "main" "dropMain") ++ "\n"
  out := out ++ (← emitTreeTU cfBu "build")
  pure (out ++ treeModuleFooter)

def readTreeInputs :
    IO (String × String × String × String × String × String × String
      × String × String) := do
  let root ← findRoot
  let a ← IO.FS.readFile (root ++ "tests/speclab/rotate_a.core")
  let b ← IO.FS.readFile (root ++ "tests/speclab/rotate_b.core")
  let d ← IO.FS.readFile (root ++ "tests/speclab/rotate_d.core")
  let c ← IO.FS.readFile (root ++ "tests/speclab/rotate_c.core")
  let rt ← IO.FS.readFile (root ++ "tests/speclab/rotate_root.core")
  let dp ← IO.FS.readFile (root ++ "tests/speclab/rotate_deep.core")
  let sp ← IO.FS.readFile (root ++ "tests/speclab/rotate_swapplant.core")
  let drp ← IO.FS.readFile (root ++ "tests/speclab/rotate_dropplant.core")
  let bu ← IO.FS.readFile (root ++ "tests/speclab/rotate_build.core")
  pure (a, b, d, c, rt, dp, sp, drp, bu)

/-! ## The CN-seed S5 emission plan (R5: swap_pair + lookup_size_shift
— the amortization rung: everything below is library assembly).

Swap parametric main: instances a/b/d are the u64-pair family with
wire bytes 1..16 / 101..116 / 201..216 — all 16 byte positions' value
triples pairwise distinct; each byte appears at TWO sites (choices[]
+ its swapped position in expected[]), so 16 parameters cover 32
substitution sites (the memcpy derived-expected mechanism at the u64
rung). Out-of-trio pin: c = (2^64-1, 0) (all-ones/all-zeros boundary
patterns).

LOOKUP IS PARKED OUT OF THE PINNED LAYER (S5 finding, registered):
the lookup dumps carry the ctype literal `'enum size'`
(load/conv_int/are_compatible/Ivalignof), and the hand-written Core
TEXT parser (CoreParser.lean — SEMANTICS SURFACE, out of the S5
worker's write scope) has no `enum TAG` arm in its quoted-ctype
grammar: `--parse-core` refuses the dump at top level (minimal
repro: any C touching an enum-typed object). The EXEC pipelines are
unaffected (the cabs-json path carries enums fine — the R5 lookup
differential lanes are green at 13 sweep samples + plants); the gap
is exactly the pinning path. The lookup fixtures
(tests/speclab/lookup_{a,b,d,c,plant}.{c,core}) stay committed as
the parked layer's inputs + reproducers; the emitter processes the
swap family only until the CoreParser gap closes (priced S,
semantics-surface, next polish arc — the S3 leak-oracle-leg
precedent).

Swap plant main + target are pinned VERBATIM from their own dump
(the plant TU shares the healthy main structure; the getarr style).

NEW pp surfaces this rung: NONE (the S4 saturation finding holds for
the swap family — any surprise is a loud error). Std closure: the
divmod 8 PLUS `ctype_width` (the u64 shift bound checks reach it —
discovered by the fail-closed unknown-function error, the S3
discipline; `wrapI_add`/`wrapI_mul`/`catch_exceptional_condition_*`
in the dumps are BUILT-IN pexpr forms (PEwrapI/PEcatch…), not std
calls). -/

/-- (valueA, valueB, valueD) → parameter name, swap trio: wire byte
i+1 ↦ `bi`. -/
def seedSwapParamTable : List ((String × String × String) × String) :=
  (List.range 16).map fun i =>
    ((toString (i + 1), toString (i + 101), toString (i + 201)),
      s!"b{i}")

def seedModuleHeader : String :=
"/-
  SpecLab.CnSeedCore — GENERATED by speclab-emit-seed (arc-15 S5).
  DO NOT EDIT.

  Kernel-transparent Lean terms for the R5 CN-seed SWAP harness
  family: swap_pair (deps/cn/tests/cn/swap_pair.c, elaborated) + the
  swap PARAMETRIC main (a function of the sixteen wire bytes b0..b15
  — 32 substitution sites, expected[] derived by the swap; digit-run
  zip over the pinned instances a/b/d, wire bytes 1..16 / 101..116 /
  201..216 — tests/speclab/pairswap_{a,b,d}.core) and the
  LOST-UPDATE PLANT pair (pairswap_plant.core, verbatim) — exactly
  as CoreParser produces them (fidelity contract of SLUnit.EmitCore).
  The std.core closure is SHARED with SpecLab.DivModCore plus ONE
  new reached declaration, `ctype_width` (the u64 shift bound
  checks; emitted below from runtime/libcore/std.core).

  THE LOOKUP FAMILY IS DELIBERATELY ABSENT: its dumps carry the
  `'enum size'` ctype literal, which the Core text parser cannot yet
  re-parse (registered S5 finding + parked price; see the seed plan
  header in SLUnit/EmitCore.lean). Its differential lanes run green
  without a pinned layer.

  Drift gate: SLUnit.SeedGateTest re-parses the pinned inputs,
  re-emits this module byte-for-byte, pins the parametric main back
  to ALL FOUR pinned healthy dumps (incl. the out-of-trio
  c = (2^64-1, 0)), and runs the assembled files through the
  generated `drive` at the pinned verdicts. Regenerate:
    .lake/build/bin/speclab-emit-seed > SpecLab/CnSeedCore.lean
-/

import Core

set_option autoImplicit false
-- parametric-main helpers bind their parameters uniformly; helpers
-- without literal sites do not reference them (generated file,
-- cosmetic)
set_option linter.unusedVariables false

namespace SpecLab.CnSeedCore

"

def seedModuleFooter : String := "\nend SpecLab.CnSeedCore\n"

def emitSeedModule (sa sb sd sc sp stdText : String) :
    Except String String := do
  let cfSA ← CoreParser.parseFile sa
  let cfSB ← CoreParser.parseFile sb
  let cfSD ← CoreParser.parseFile sd
  let cfSC ← CoreParser.parseFile sc
  let cfSP ← CoreParser.parseFile sp
  let std ← CoreParser.parseFile stdText
  -- the target must be literally identical across the parametric
  -- family's four dumps (identical main structure ⇒ identical fresh
  -- numbering — the S3 coupling finding)
  let st ← declText cfSA "swap_pair"
  for (lbl, cf) in [("b", cfSB), ("d", cfSD), ("c", cfSC)] do
    if (← declText cf "swap_pair") ≠ st then
      throw s!"emitSeedModule: 'swap_pair' differs in dump {lbl}"
  -- the parametric main: per-instance FULL blocks zipped whole (the
  -- S2 mechanism; hoisting decisions literal-independent)
  let (swapMainSymA, _) ← findDecl cfSA "main"
  let ssig := " (b0 b1 b2 b3 b4 b5 b6 b7 b8 b9 b10 b11 b12 b13 b14 b15 : Int)"
  let sapp := " b0 b1 b2 b3 b4 b5 b6 b7 b8 b9 b10 b11 b12 b13 b14 b15"
  let bsa ← emitMainBlockP cfSA "swapMainParamDecl" ssig sapp
  let bsb ← emitMainBlockP cfSB "swapMainParamDecl" ssig sapp
  let bsd ← emitMainBlockP cfSD "swapMainParamDecl" ssig sapp
  let spblock ← zipParamWith seedSwapParamTable 32 bsa bsb bsd
  let mut out := seedModuleHeader
  out := out ++ (← emitDeclH cfSA "swap_pair" "swapPair") ++ "\n"
  out := out ++ s!"/-- swap `main` (parametric family): symbol, as interned by CoreParser. -/\n"
  out := out ++ s!"def swapMainSym : sym :=\n  {← ppSym swapMainSymA}\n\n"
  out := out ++ "/- swap `main`, PARAMETRIC in the sixteen wire bytes (see the module\nheader; expected[] sites are DERIVED — the swapped positions share\nthe parameters; hoisted helpers below carry the binders).\n`swapMainParamDecl 1 2 ... 16` is the parsed main of the pinned\ninstance a, byte-for-byte (SLUnit.SeedGateTest). -/\n"
  out := out ++ spblock ++ "\n"
  out := out ++ (← emitDeclH cfSP "swap_pair" "swapPairPlant") ++ "\n"
  out := out ++ (← emitDeclH cfSP "main" "swapMainPlant") ++ "\n"
  out := out ++ (← emitDeclH std "ctype_width" "ctypeWidth") ++ "\n"
  pure (out ++ seedModuleFooter)

def readSeedInputs :
    IO (String × String × String × String × String × String) := do
  let root ← findRoot
  let sa ← IO.FS.readFile (root ++ "tests/speclab/pairswap_a.core")
  let sb ← IO.FS.readFile (root ++ "tests/speclab/pairswap_b.core")
  let sd ← IO.FS.readFile (root ++ "tests/speclab/pairswap_d.core")
  let sc ← IO.FS.readFile (root ++ "tests/speclab/pairswap_c.core")
  let sp ← IO.FS.readFile (root ++ "tests/speclab/pairswap_plant.core")
  let std ← IO.FS.readFile (root ++ "runtime/libcore/std.core")
  pure (sa, sb, sd, sc, sp, std)
