...
#   fork does not touch, passed to the gate AS A RAW COMMIT HASH (no ref is created: a shared-namespace
#   ref would race between concurrent self-tests — review P3-1). The gate compares against the pinned
#   merge-base, so this must be OK, not drift. The dangling objects are gc-collectable.
