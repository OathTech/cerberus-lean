...
if $RECORD_BASELINE; then
    sort "$OUTPUT_DIR/baseline.new" -o "$OUTPUT_DIR/baseline.new"
    {
        ...
        echo "# §4/§5, record docs/2026-09-04_zero-discrepancy-Z2-record.md): zd-z2m01-* (aligned_alloc(0,·):"
        echo "# oracle Division_by_zero at std.core:385 rem_t), zd-z2cp01-strtod-inf (libc.core \`inf\` literal),"
        echo "# zd-z2i01-cerbty-int32-uac (IntN_t aliasing), zd-z2f01-lseek-whence (SibylFS EINVAL) — each pinned"
        echo "# at the CURRENT Lean token BEFORE its fix (ORACLE_CRASH / DIFF) and re-recorded by the fix commit"
        echo "# that cites it (MATCH, or MATCH | L=CRASH for the both-crash rows)."
