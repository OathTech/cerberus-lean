  ...
  Core_parser_driver.parse core_stdlib filename >>= function
    | Core_parser_util.Rfile (sym_main, globs, funs, tagDefs) ->
        ...
        return {
           ...
           Core.enumDefs= Pmap.empty Symbol.symbol_compare; (* program-data parameters E-A: dead on this target (the registry is read directly); a .core input has no enum declarations *)
...
let read_core_object (conf, io) ?(is_lib=false) (core_stdlib, core_impl) filename =
  ...
  let core_file = { main=    dump.dump_main;
    ...
    enumDefs= Pmap.empty sym_compare; (* program-data parameters E-A: dead on this target; a .co object carries no enum map *)
